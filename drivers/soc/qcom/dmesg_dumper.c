			 ...
			 size_t count, loff_t *ppos)
{
	...
	if (last_dump) {
		...
		len += sprintf(dump_status + len, "\tpvm timing: %llu.%llu s\n",
			       hdr->pvm_read_timing / 1000000000ULL,
			       hdr->pvm_read_timing % 1000000000ULL);
		len += sprintf(dump_status + len,
			       "\tread len: %llu, svm_suspend: %d\n",
			       hdr->pvm_read_len, hdr->pvm_read_suspend);
		len += sprintf(dump_status + len, "[svm info]\n");
		len += sprintf(dump_status + len,
			       "\tsvm suspend timing: %llu.%llu s\n\n",
			       hdr->svm_suspend_timing / 1000000000ULL,
			       hdr->svm_suspend_timing % 1000000000ULL);
	...
	if (hdr->svm_is_suspend && hdr->svm_dump_len < 1024) {
		if (fresh_read) {
			...
			len += sprintf(
				dump_status + len,
				"[skip read as svm is in suspend and log size less than 1k]\n");
			(void)copy_to_user(buf, dump_status, len);
	...
	if (ret) {
		if (fresh_read) {
			...
			len += sprintf(
				dump_status + len,
				" [gh dbl kick failed with %d, svm might abnormal\n",
				ret);
			(void)copy_to_user(buf, dump_status, len);
			return len;
		}
	...
	if (!ret) {
		if (fresh_read) {
			...
			len += sprintf(
				dump_status + len,
				" [gh dbl wait timeout with %d, svm might abnormal\n",
				ret);
			(void)copy_to_user(buf, dump_status, len);
			return len;
		}
...
static int qcom_ddump_alive_log_probe(struct qcom_dmesg_dumper *qdd)
{
	...
	if (qdd->primary_vm) {
		...
		dent = proc_create_data(DDUMP_PROFS_NAME, 0444, NULL,
					&ddump_proc_ops, qdd);
		if (!dent) {
