...

{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret =
			...
			i2c_smbus_write_byte_data(awinic->i2c, reg_addr,
						  reg_data);
...

{
	...
	if (awinic->name == AW8697) {
		...
		awinic->aw8697->work_queue = create_singlethread_workqueue(
			"aw8697_vibrator_work_queue");
