...
static void sih688x_software_reset(sih_haptic_t *sih_haptic)
{
	...
	haptic_regmap_write(sih_haptic->regmapp.regmapping, SIH688X_REG_ID,
			    SIH_I2C_OPERA_BYTE_ONE, &reg_value);
...
static void sih688x_detect_done_int(sih_haptic_t *sih_haptic, bool flag)
{
	hp_info("%s:enter! flag:%d\n", __func__, flag);
	if (flag) {
		haptic_regmap_update_bits(
			sih_haptic->regmapp.regmapping, SIH688X_REG_SYSINTM2,
			SIH_SYSINT2_BIT_F0_DETECT_DONE_INT_MASK,
...
static void sih688x_set_go_enable(sih_haptic_t *sih_haptic, uint8_t play_mode)
{
...
static void sih688x_set_go_disable(sih_haptic_t *sih_haptic)
{
	...
	hp_info("%s:enter!\n", __func__);
	haptic_regmap_write(sih_haptic->regmapp.regmapping, SIH688X_REG_GO,
...
static void sih688x_play_go(sih_haptic_t *sih_haptic, bool flag)
{
	...
	if (flag) {
		...
		sih_haptic->chip_ipara.interval_us = ktime_to_us(
			ktime_sub(sih_haptic->chip_ipara.kcur_time,
...
static void sih688x_clear_interrupt_state(sih_haptic_t *sih_haptic)
{
	...
	hp_info("%s:enter\n", __func__);
	haptic_regmap_read(sih_haptic->regmapp.regmapping, SIH688X_REG_SYSINT,
			   SIH_I2C_OPERA_BYTE_ONE, &reg_val);
}
...
static void sih688x_set_gain(sih_haptic_t *sih_haptic, uint8_t gain)
{
	...
	haptic_regmap_write(sih_haptic->regmapp.regmapping,
			    SIH688X_REG_PWM_PRE_GAIN, SIH_I2C_OPERA_BYTE_ONE,
			    &gain);
}
...
static void sih688x_f0_cali(sih_haptic_t *sih_haptic)
{
	...
	if (sih_haptic->detect.tracking_f0 > SIH688X_F0_VAL_MAX ||
		...
		code = 0;
		XM_HAP_F0_CAL_EXCEPTION(sih_haptic->detect.tracking_f0,
					sih_haptic->detect.cali_target_value,
					"F0 Calibration failed");
	} else {
		tmp = (int32_t)(sih_haptic->detect.tracking_f0 *
				SIH688X_F0_CAL_COE /
				sih_haptic->detect.cali_target_value);
...
static void sih688x_set_play_mode(sih_haptic_t *sih_haptic, uint8_t play_mode)
{
	uint8_t reg_val = 0;
	...
	case SIH_IDLE_MODE:
		...
		reg_val =
			SIH_GO_BIT_RAM_GO_DISABLE | SIH_GO_BIT_RTP_GO_DISABLE |
			SIH_GO_BIT_F0_SEQ_GO_DISABLE | SIH_GO_BIT_STOP_TRIG_EN;
		haptic_regmap_write(sih_haptic->regmapp.regmapping,
				    SIH688X_REG_GO, SIH_I2C_OPERA_BYTE_ONE,
				    &reg_val);
		usleep_range(2000, 2500);
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
					  SIH688X_REG_SYSCTRL1,
...
static void sih688x_set_brk_bst_vol(sih_haptic_t *sih_haptic, uint32_t bst_vol)
{
	uint32_t tmp = 0;
	...
	haptic_regmap_write(sih_haptic->regmapp.regmapping, SIH688X_REG_V_BOOST,
			    SIH_I2C_OPERA_BYTE_ONE, &write_value);
	haptic_regmap_write(sih_haptic->regmapp.regmapping, SIH688X_REG_V_BOOST,
			    SIH_I2C_OPERA_BYTE_ONE, &write_value);
}

static void sih688x_set_drv_bst_vol(sih_haptic_t *sih_haptic, uint32_t drv_bst)
{
	uint8_t bst_vol = 0;
	...
	*/
	bst_reg_val = (uint8_t)(((drv_bst - SIH688X_DRV_BOOST_BASE) *
				 SIH688X_DRV_BOOST_SETP_COE) /
				SIH688X_DRV_BOOST_SETP);

	haptic_regmap_write(sih_haptic->regmapp.regmapping,
			    SIH688X_REG_ANA_CTRL3, SIH_I2C_OPERA_BYTE_ONE,
			    &bst_reg_val);
	...
	if (drv_bst >= SIH_ANA_CTRL_BST_LEVEL_6 &&
		...
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
					  SIH688X_REG_ANA_CTRL5,
					  SIH_ANA_CTRL5_BST_IOS_SEL_O_MASK,
					  SIH_ANA_CTRL5_BST_IOS_SEL_6_8);
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
					  SIH688X_REG_ANA_CTRL5,
					  SIH_ANA_CTRL5_BST_OCP_VRSEL_O_MASK,
					  SIH_ANA_CTRL5_BST_OCP_VRSEL_6_8);
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
					  SIH688X_REG_ANA_CTRL6,
					  SIH_ANA_CTRL6_BST_ZCD_IOS_O_MASK,
...
static void sih688x_set_low_power_mode(sih_haptic_t *sih_haptic, bool flag)
{
	if (flag) {
		haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
					  SIH688X_REG_SYSCTRL1,
...
static void sih688x_set_brk_state(sih_haptic_t *sih_haptic, uint8_t mode,
				  bool flag)
{
	uint8_t brk_en;
	uint8_t brk_mask;
	...
	haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
				  SIH688X_REG_MAIN_STATE_CTRL, brk_mask,
				  brk_en);
}
...
static size_t sih688x_get_brk_state(sih_haptic_t *sih_haptic, char *buf)
{
	...
	haptic_regmap_read(sih_haptic->regmapp.regmapping,
			   SIH688X_REG_MAIN_STATE_CTRL, SIH_I2C_OPERA_BYTE_ONE,
			   &reg_val);
...
static void sih688x_set_pwm_rate(sih_haptic_t *sih_haptic, uint8_t sample_rpt,
				 uint8_t sample_en)
{
	switch (sample_rpt) {
...
static size_t sih688x_get_pwm_rate(sih_haptic_t *sih_haptic, char *buf)
{
	...
	haptic_regmap_read(sih_haptic->regmapp.regmapping,
			   SIH688X_REG_PWM_UP_SAMPLE_CTRL,
			   SIH_I2C_OPERA_BYTE_ONE, &pwm_state);
...
static bool sih688x_if_chip_is_detect_done(sih_haptic_t *sih_haptic)
{
	...
	haptic_regmap_read(sih_haptic->regmapp.regmapping, SIH688X_REG_SYSINT2,
			   SIH_I2C_OPERA_BYTE_ONE, &reg_val);
...
static bool sih688x_if_chip_is_mode(sih_haptic_t *sih_haptic, uint8_t mode)
{
	...
	switch (mode) {
	case SIH_IDLE_MODE:
		flag = (reg_val & SIH_SYSSST_BIT_STANDBY) ==
		       SIH_SYSSST_BIT_STANDBY;
		break;
	case SIH_RAM_MODE:
	case SIH_RAM_LOOP_MODE:
		flag = (reg_val & SIH_SYSSST_BIT_RAM_STATE) ==
		       SIH_SYSSST_BIT_RAM_STATE;
		break;
	case SIH_RTP_MODE:
		flag = (reg_val & SIH_SYSSST_BIT_RTP_STATE) ==
		       SIH_SYSSST_BIT_RTP_STATE;
		break;
	case SIH_TRIG_MODE:
	...
	while (1) {
		if (!sih688x_if_chip_is_standby(sih_haptic)) {
			...
			sih_haptic->chip_ipara.interval_us = ktime_to_us(
				ktime_sub(sih_haptic->chip_ipara.kcur_time,
					  sih_haptic->chip_ipara.kpre_time));
			if (sih_haptic->chip_ipara.interval_us >
			    SIH_PROTECTION_TIME)
				break;
...
static int sih688x_efuse_check(sih_haptic_t *sih_haptic)
{
	...
	haptic_regmap_write(sih_haptic->regmapp.regmapping,
			    SIH688X_REG_EFUSE_CTRL, SIH_I2C_OPERA_BYTE_ONE,
			    &write_value);
	haptic_regmap_bulk_read(sih_haptic->regmapp.regmapping,
				SIH688X_REG_EFUSE_RDATA0,
				SIH_I2C_OPERA_BYTE_FOUR, efuse_data);
	efuse_data[3] = efuse_data[3] & 0x0F;
	haptic_regmap_read(sih_haptic->regmapp.regmapping,
			   SIH688X_REG_EFUSE_RDATA4, SIH_I2C_OPERA_BYTE_ONE,
			   &efuse_data[4]);
	haptic_regmap_read(sih_haptic->regmapp.regmapping,
			   SIH688X_REG_EFUSE_RDATA3, SIH_I2C_OPERA_BYTE_ONE,
			   &crc4_value);
...
static int sih688x_check_ram_data(sih_haptic_t *sih_haptic, uint8_t *cont_data,
				     ...
				     haptic_container_t *sih_cont)
{
	...
	hp_info("%s:fifo[0] = %d,[1] = %d\n", __func__, fifo_addr[0],
		fifo_addr[1]);
	hp_info("%s:fifo[2] = %d,[3] = %d\n", __func__, fifo_addr[2],
		fifo_addr[3]);

	haptic_regmap_bulk_write(sih_haptic->regmapp.regmapping,
				 SIH688X_REG_BASE_ADDRH, SIH_I2C_OPERA_BYTE_TWO,
				 &sih_cont->data[shift]);
	haptic_regmap_bulk_write(sih_haptic->regmapp.regmapping,
				 SIH688X_REG_RTPCFG1, SIH_I2C_OPERA_BYTE_TWO,
				 fifo_addr);
	haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
				  SIH688X_REG_RTPCFG3,
	...
	haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
				  SIH688X_REG_RTPCFG3,
				  SIH_RTPCFG3_BIT_FIFO_AEH_MASK, fifo_addr[3]);

	/* ram */
	haptic_regmap_bulk_write(sih_haptic->regmapp.regmapping,
				 SIH688X_REG_RAMADDRH, SIH_I2C_OPERA_BYTE_TWO,
				 &sih_cont->data[shift]);
	...
	if (!ram_data) {
		...
		while (i < sih_cont->len) {
			...
			haptic_regmap_bulk_read(sih_haptic->regmapp.regmapping,
						SIH688X_REG_RAMDATA, len,
						ram_data);
			ret = sih688x_check_ram_data(
				sih_haptic, &sih_cont->data[i], ram_data, len);
			if (ret < 0)
				break;
			i += len;
		}
...
static void sih688x_set_wav_seq(sih_haptic_t *sih_haptic, uint8_t seq,
				uint8_t wave)
{
	hp_info("%s:seq:%d,wave:%d\n", __func__, seq, wave);
	haptic_regmap_write(sih_haptic->regmapp.regmapping,
			    SIH688X_REG_WAVESEQ1 + seq, SIH_I2C_OPERA_BYTE_ONE,
			    &wave);
}
					...
					uint8_t *wave_addr)
{
	...
	haptic_regmap_bulk_read(sih_haptic->regmapp.regmapping,
				SIH688X_REG_RAMDATA, SIH_I2C_OPERA_BYTE_THREE,
				reg_array);
...
static void sih688x_set_wav_loop(sih_haptic_t *sih_haptic, uint8_t seq,
				 uint8_t loop)
{
	uint8_t offset;
	...
	haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
				  SIH688X_REG_WAVELOOP1 + (seq / 2),
				  WAVELOOP_SEQ_EVEN_MASK << offset,
				  loop << offset);
}
...
static void sih688x_get_wav_loop(sih_haptic_t *sih_haptic)
{
	...
	haptic_regmap_bulk_read(sih_haptic->regmapp.regmapping,
				SIH688X_REG_WAVELOOP1, SIH_I2C_OPERA_BYTE_FOUR,
				reg_val);

	for (i = 0; i < SIH_HAPTIC_SEQUENCER_SIZE / 2; i++) {
		sih_haptic->ram.loop[i * 2 + 0] = (reg_val[i] >> 4) &
						  WAVELOOP_SEQ_EVEN_MASK;
		sih_haptic->ram.loop[i * 2 + 1] = (reg_val[i] >> 0) &
						  WAVELOOP_SEQ_EVEN_MASK;
	}
}

static void sih688x_set_wav_main_loop(sih_haptic_t *sih_haptic, uint8_t loop)
{
	hp_info("%s:main loop = %d\n", __func__, loop);
	haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
				  SIH688X_REG_MAINLOOP,
				  SIH_MAINLOOP_BIT_MAIN_LOOP_MASK, loop);
}
...
static void sih688x_get_wav_main_loop(sih_haptic_t *sih_haptic)
{
	...
	haptic_regmap_read(sih_haptic->regmapp.regmapping, SIH688X_REG_MAINLOOP,
			   SIH_I2C_OPERA_BYTE_ONE, &reg_val);
...
static void sih688x_set_ram_seq_gain(sih_haptic_t *sih_haptic, uint8_t wav,
				     uint8_t gain)
{
	uint8_t offset = 0;
...
static size_t sih688x_get_ram_seq_gain(sih_haptic_t *sih_haptic, char *buf)
{
	...
	haptic_regmap_bulk_read(sih_haptic->regmapp.regmapping,
				...
				SIH_I2C_OPERA_BYTE_FOUR, reg_val);
...
static size_t sih688x_write_rtp_data(sih_haptic_t *sih_haptic, uint8_t *data,
				     uint32_t len)
{
	int ret = -1;
...
static ssize_t sih688x_get_cont_para(sih_haptic_t *sih_haptic, uint8_t flag,
				     char *buf)
{
	ssize_t len = 0;
	uint8_t reg_addr_start = SIH688X_CONT_PARA_SEQ0;
...
{ /* index enable polar trig_mode boost_bypass p_id n_id */
	...
	haptic_regmap_update_bits(
		sih_haptic->regmapp.regmapping, SIH688X_REG_TRIG_CTRL1,
	...
	haptic_regmap_update_bits(
		sih_haptic->regmapp.regmapping, SIH688X_REG_TRIG_CTRL2,
...
static size_t sih688x_trig_para_get(sih_haptic_t *sih_haptic, char *buf)
{
	...
	haptic_regmap_bulk_read(sih_haptic->regmapp.regmapping,
				SIH688X_REG_TRIG_CTRL1, SIH_I2C_OPERA_BYTE_TWO,
				trig_ctrl);
...
static void sih688x_set_detect_state(sih_haptic_t *sih_haptic, uint8_t mode,
				     bool flag)
{
	uint8_t f0_en;
	uint8_t f0_mask;
				       ...
				       uint8_t play_mode)
{
	...
	if (detect_flag) {
		haptic_regmap_bulk_write(sih_haptic->regmapp.regmapping,
					 SIH688X_REG_T_LAST_MONITOR_L,
					 SIH_I2C_OPERA_BYTE_TWO, reg_val);
...
static void sih688x_get_lra_resistance(sih_haptic_t *sih_haptic)
{
	...
	for (i = 0; i < SIH688X_RL_CONFIG_REG_NUM; i++) {
		haptic_regmap_read(sih_haptic->regmapp.regmapping,
				   save_reg_addr[i], SIH_I2C_OPERA_BYTE_ONE,
				   &save_reg_val[i]);
	}
	...
	while (time--) {
		if (sih688x_if_chip_is_done(sih_haptic)) {
			...
			haptic_regmap_read(sih_haptic->regmapp.regmapping,
					   ...
					   SIH_I2C_OPERA_BYTE_ONE, &rl_high);
			haptic_regmap_read(sih_haptic->regmapp.regmapping,
					   SIH688X_REG_ADC_RL_DATA_L,
					   SIH_I2C_OPERA_BYTE_ONE, &rl_low);
			...
			if (rl_rawdata > SIH688X_RL_SAR_CODE_DIVIDER) {
				...
				*
				*/
				sih_haptic->detect.resistance =
					rl_rawdata * SIH688X_B0_RL_AMP_COE /
					SIH688X_B0_RL_DIV_COE;
			} else {
				/* calc rl */
				/*
				* rl calc formulation:
	...
	while (time--) {
		if (sih688x_if_chip_is_done(sih_haptic)) {
			...
			haptic_regmap_read(sih_haptic->regmapp.regmapping,
					   ...
					   SIH_I2C_OPERA_BYTE_ONE, &vbat_high);
			haptic_regmap_read(sih_haptic->regmapp.regmapping,
					   SIH688X_REG_ADC_VBAT_DATA_L,
					   SIH_I2C_OPERA_BYTE_ONE, &vbat_low);
			vbat_raw_data =
				(uint32_t)((vbat_high & 0xf0) << 4 | vbat_low);
			/* calc vbat */
			/*
...
static void sih688x_get_detect_f0(sih_haptic_t *sih_haptic)
{
	...
	if (sih_haptic->detect.rtp_detect_en |
	    sih_haptic->detect.ram_detect_en |
...
static void sih688x_vbat_comp(sih_haptic_t *sih_haptic)
{
	...
	*/
	comp_gain =
		SIH688X_STANDARD_VBAT * sih_haptic->chip_ipara.gain / curr_vbat;
...
static void sih688x_osc_cali(sih_haptic_t *sih_haptic)
{
	...
	case SIH_PWM_UP_SAMPLE_CTRL_BIT_PWM_UP_ONE_TIME:
		sih_haptic->osc_para.theory_time =
			sih_haptic->osc_para.osc_rtp_len *
			SIH688X_OSC_RTL_DATA_LEN / SIh688X_PWM_SAMPLE_48KHZ;
		break;
	case SIH_PWM_UP_SAMPLE_CTRL_BIT_PWM_UP_TWO_TIME:
		sih_haptic->osc_para.theory_time =
			sih_haptic->osc_para.osc_rtp_len *
			SIH688X_OSC_RTL_DATA_LEN / SIh688X_PWM_SAMPLE_24KHZ;
		break;
	case SIH_PWM_UP_SAMPLE_CTRL_BIT_PWM_UP_FOUR_TIME:
		sih_haptic->osc_para.theory_time =
			sih_haptic->osc_para.osc_rtp_len *
			SIH688X_OSC_RTL_DATA_LEN / SIh688X_PWM_SAMPLE_12KHZ;
		break;
	default:
		break;
	}
...
static void sih688x_init(sih_haptic_t *sih_haptic)
{
	...
	/* idle cnt reg init */
	haptic_regmap_write(sih_haptic->regmapp.regmapping,
			    SIH688X_REG_IDLE_DEL_CNT, SIH_I2C_OPERA_BYTE_ONE,
			    &reg_val);
	...
	haptic_regmap_update_bits(sih_haptic->regmapp.regmapping,
				  SIH688X_REG_SMOOTH_F0_WINDOW_OUT,
				  SIH_WAIT_FIFO_DETECT_MASK,
				  SIH_WAIT_FIFO_DETECT_OFF);

	/* brk vboost init */
	haptic_regmap_read(sih_haptic->regmapp.regmapping, SIH688X_REG_V_BOOST,
			   SIH_I2C_OPERA_BYTE_ONE, &reg_val);
	/**
	...
	*/
	sih_haptic->chip_ipara.brk_vboost = (((uint32_t)reg_val) << 8) *
					    SIH688X_BRK_VBOOST_COE /
					    SIH688X_VBOOST_MUL_COE;
