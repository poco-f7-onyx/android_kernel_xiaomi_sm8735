			     ...
			     struct device_node *np)
{
	...
	if (val != 0) {
		...
		XM_HAP_F0_PROTECT_EXCEPTION(
			0, "SIH：haptics_get_closeloop_lra_period");
	}
...
static int sih_acquire_prepare_res(struct device *dev, sih_haptic_t *sih_haptic)
{
	int ret = -1;
					  ...
					  haptic_container_t *rtp_cont)
{
	...
	if (sih_haptic->chip_ipara.is_custom_wave == 0) {
		...
		hp_err("%s: cont_len = %d,sih_haptic->rtp.rtp_cnt = %d,inject_data_cnt = %d\n",
		       __func__, cont_len, sih_haptic->rtp.rtp_cnt,
		       inject_data_cnt);
		if (buf_len > 0) {
			ret = sih_haptic->hp_func->write_rtp_data(
				sih_haptic,
				&rtp_cont->data[sih_haptic->rtp.rtp_cnt],
				buf_len);
			if (ret < 0) {
				sih_haptic->hp_func->stop(sih_haptic);
				sih_haptic->hp_func->set_rtp_aei(sih_haptic,
								 false);
				sih_haptic->rtp.rtp_init = false;
				sih_haptic->rtp.rtp_cnt = 0;
				pm_relax(sih_haptic->dev);
		...
		if ((sih_haptic->rtp.rtp_cnt == cont_len) ||
		    sih_haptic->hp_func->if_chip_is_mode(sih_haptic,
							 SIH_IDLE_MODE)) {
			if (sih_haptic->rtp.rtp_cnt != cont_len)
		...
		if (buf_len > 0) {
			ret = sih_haptic->hp_func->write_rtp_data(
				sih_haptic, rtp_cont->data, buf_len);
			if (ret < 0) {
				sih_haptic->hp_func->stop(sih_haptic);
				sih_haptic->hp_func->set_rtp_aei(sih_haptic,
								 false);
...
static irqreturn_t sih_irq_isr(int irq, void *data)
{
	...
	if (sih_haptic->hp_func->get_rtp_fifo_empty_state(sih_haptic)) {
		if (sih_haptic->rtp.rtp_init) {
			while ((!sih_haptic->hp_func->get_rtp_fifo_full_state(
				       sih_haptic)) &&
			       (sih_haptic->chip_ipara.play_mode ==
				SIH_RTP_MODE) &&
			       !atomic_read(
				       &sih_haptic->rtp.exit_in_rtp_loop)) {
	...
	/* detect */
	if ((sih_haptic->detect.trig_detect_en |
	     sih_haptic->detect.ram_detect_en |
	     sih_haptic->detect.rtp_detect_en |
	     sih_haptic->detect.cont_detect_en) &&
...
static int sih_acquire_irq_res(struct device *dev, sih_haptic_t *sih_haptic)
{
	...
	ret = devm_request_threaded_irq(
		dev, gpio_to_irq(sih_haptic->chip_attr.irq_gpio), NULL,
		sih_irq_isr, irq_flags, sih_haptic->soft_frame.vib_dev.name,
		sih_haptic);
...
static void sih_rtp_play_func(sih_haptic_t *sih_haptic, uint8_t mode)
{
	...
	while (!sih_haptic->hp_func->get_rtp_fifo_full_state(sih_haptic) &&
		...
		if (sih_haptic->chip_ipara.is_custom_wave == 0) {
				...
				if (mode == SIH_RTP_OSC_PLAY) {
					if (sih_haptic->osc_para.start_flag) {
						sih_haptic->osc_para.kstart =
							ktime_get();
						sih_haptic->osc_para.start_flag =
							false;
					}
				}
				if (buf_len > 0) {
					sih_haptic->hp_func->write_rtp_data(
						sih_haptic,
						&rtp_cont->data[sih_haptic->rtp
									.rtp_cnt],
						buf_len);
					sih_haptic->rtp.rtp_cnt += buf_len;
			...
			if ((sih_haptic->rtp.rtp_cnt == cont_len) &&
			    sih_haptic->hp_func->if_chip_is_mode(
				    sih_haptic, SIH_IDLE_MODE)) {
				if (sih_haptic->rtp.rtp_cnt != cont_len)
...
static void sih_rtp_local_work(sih_haptic_t *sih_haptic, uint8_t mode)
{
	...
	if (ret < 0) {
		hp_err("%s:fail to read %s\n", __func__,
		       awinic_rtp_name[rtp_file_index]);
		sih_chip_state_recovery(sih_haptic);
		mutex_unlock(&sih_haptic->rtp.rtp_lock);
		return;
	}
	...
	while (cnt--) {
		if (sih_haptic->hp_func->if_chip_is_mode(sih_haptic,
							 SIH_RTP_MODE)) {
			rtp_work_flag = true;
			hp_info("%s:rtp go!\n", __func__);
			break;
...
static void get_ram_num(sih_haptic_t *sih_haptic)
{
	...
	sih_haptic->ram.wave_num =
		(first_wave_addr - sih_haptic->ram.base_addr - 1) / 4;
...
static void sih_ram_load(const struct firmware *cont, void *context)
{
	...
	if (ret) {
		...
		sih_haptic->ram.len =
			sih_haptic_fw->len - sih_haptic->ram.ram_shift;
...
static void sih_ram_play(sih_haptic_t *sih_haptic, uint8_t mode)
{
	...
	sih_haptic->hp_func->set_wav_seq(sih_haptic, 0,
					 sih_haptic->chip_ipara.effect_id + 1);
...
{
	sih_haptic_t *sih_haptic =
		container_of(work, sih_haptic_t, ram.gain_work);
	...
	if (sih_haptic->chip_ipara.new_gain >= 0x7FFF)
		...
		sih_haptic->chip_ipara.gain =
			(sih_haptic->chip_ipara.new_gain - 16383) / 128;

	if (sih_haptic->chip_ipara.gain < 0x1E)
		sih_haptic->chip_ipara.gain = 0x1E; /*30 */
	hp_info("%s:new_gain = %x level = %x\n", __func__,
		sih_haptic->chip_ipara.new_gain, sih_haptic->chip_ipara.gain);
...
int sih_upload_effect(struct input_dev *dev, struct ff_effect *effect,
		      struct ff_effect *old)
{
	...
	if (sih_haptic->chip_ipara.effect_type == FF_CONSTANT) {
		...
		if (sih_haptic->chip_ipara.effect_id <
		    sih_haptic->chip_ipara.effect_id_boundary) {
...
int sih_playback(struct input_dev *dev, int effect_id, int val)
{
	sih_haptic_t *sih_haptic = input_get_drvdata(dev);
	int rc = 0;

	hp_info("%s: effect_id=%d , action_mode = %d val = %d\n", __func__,
		sih_haptic->chip_ipara.effect_id, sih_haptic->ram.action_mode,
		val);
			...
			char *buf)
{
	...
	for (i = 0; i < SIH_HAPTIC_SEQUENCER_SIZE; i++) {
		len += snprintf(buf + len, PAGE_SIZE - len, "seq%d = %d\n", i,
				sih_haptic->ram.seq[i]);
	}
			 ...
			 const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		sih_haptic->hp_func->set_wav_seq(
			sih_haptic, (uint8_t)databuf[0], (uint8_t)databuf[1]);
		mutex_unlock(&sih_haptic->lock);
	}
			 ...
			 const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		haptic_regmap_write(sih_haptic->regmapp.regmapping,
				    (uint8_t)databuf[0], SIH_I2C_OPERA_BYTE_ONE,
				    &val);
	}
...
static ssize_t auto_pvdd_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
...
static ssize_t effect_id_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
			  ...
			  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		sih_haptic->hp_func->set_wav_loop(
			sih_haptic, (uint8_t)databuf[0], (uint8_t)databuf[1]);
		mutex_unlock(&sih_haptic->lock);
	}
...
static ssize_t cont_go_store(struct device *dev, struct device_attribute *attr,
			     const char *buf, size_t count)
{
	sih_haptic_t *sih_haptic = NULL;
	uint32_t val = 0;
...
static ssize_t cont_seq0_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
			       ...
			       size_t count)
{
	...
	if (sscanf(buf, "%x %x %x", &databuf[0], &databuf[1], &databuf[2]) ==
	    3) {
		param[0] = (uint8_t)databuf[0];
		param[1] = (uint8_t)databuf[1];
		param[2] = (uint8_t)databuf[2];
		mutex_lock(&sih_haptic->lock);
		sih_haptic->hp_func->set_cont_para(
			sih_haptic, SIH688X_CONT_PARA_SEQ0, param);
		mutex_unlock(&sih_haptic->lock);
	}
...
static ssize_t cont_seq1_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
			       ...
			       size_t count)
{
	...
	if (sscanf(buf, "%x %x %x", &databuf[0], &databuf[1], &databuf[2]) ==
		...
		mutex_lock(&sih_haptic->lock);
		sih_haptic->hp_func->set_cont_para(
			sih_haptic, SIH688X_CONT_PARA_SEQ1, param);
		mutex_unlock(&sih_haptic->lock);
	}
...
static ssize_t cont_seq2_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
			       ...
			       size_t count)
{
	...
	if (sscanf(buf, "%x %x %x", &databuf[0], &databuf[1], &databuf[2]) ==
		...
		mutex_lock(&sih_haptic->lock);
		sih_haptic->hp_func->set_cont_para(
			sih_haptic, SIH688X_CONT_PARA_SEQ2, param);
		mutex_unlock(&sih_haptic->lock);
	}
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	len = sih_haptic->hp_func->get_cont_para(
		sih_haptic, SIH688X_CONT_PARA_ASMOOTH, buf);
				  ...
				  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%x %x %x", &databuf[0], &databuf[1], &databuf[2]) ==
		...
		mutex_lock(&sih_haptic->lock);
		sih_haptic->hp_func->set_cont_para(
			sih_haptic, SIH688X_CONT_PARA_ASMOOTH, param);
		mutex_unlock(&sih_haptic->lock);
	}
				 ...
				 size_t count)
{
	...
	if (sscanf(buf, "%x %x %x", &databuf[0], &databuf[1], &databuf[2]) ==
		...
		mutex_lock(&sih_haptic->lock);
		sih_haptic->hp_func->set_cont_para(
			sih_haptic, SIH688X_CONT_PARA_TH_LEN, param);
		mutex_unlock(&sih_haptic->lock);
	}
				 ...
				 size_t count)
{
	...
	if (sscanf(buf, "%x %x %x", &databuf[0], &databuf[1], &databuf[2]) ==
		...
		mutex_lock(&sih_haptic->lock);
		sih_haptic->hp_func->set_cont_para(
			sih_haptic, SIH688X_CONT_PARA_TH_NUM, param);
		mutex_unlock(&sih_haptic->lock);
	}
				...
				size_t count)
{
	...
	if (sscanf(buf, "%x %x %x", &databuf[0], &databuf[1], &databuf[2]) ==
		...
		mutex_lock(&sih_haptic->lock);
		sih_haptic->hp_func->set_cont_para(
			sih_haptic, SIH688X_CONT_PARA_AMPLI, param);
		mutex_unlock(&sih_haptic->lock);
	}
...
static ssize_t ram_store(struct device *dev, struct device_attribute *attr,
			 const char *buf, size_t count)
{
	sih_haptic_t *sih_haptic = NULL;
...
static ssize_t f0_value_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	ssize_t len = 0;
	sih_haptic_t *sih_haptic = NULL;
...
static ssize_t trig_para_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
			       ...
			       size_t count)
{
	...
	if (sscanf(buf, "%x %x %x %x %x %x %x", &databuf[0], &databuf[1],
...
static ssize_t low_power_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
				...
				size_t count)
{
	...
	if (val < SIH688X_DRV_VBOOST_MIN * SIH688X_DRV_VBOOST_COEFFICIENT) {
		...
		sih_haptic->chip_ipara.drv_vboost =
			SIH688X_DRV_VBOOST_MIN * SIH688X_DRV_VBOOST_COEFFICIENT;
	} else if (val >
		   SIH688X_DRV_VBOOST_MAX * SIH688X_DRV_VBOOST_COEFFICIENT) {
		hp_info("%s:drv_vboost is too high,set to 110:%d", __func__,
			val);
		sih_haptic->chip_ipara.drv_vboost =
			SIH688X_DRV_VBOOST_MAX * SIH688X_DRV_VBOOST_COEFFICIENT;
	}
...
static ssize_t burst_rw_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
...
static ssize_t burst_rw_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	uint8_t reg_val = 0;
	uint32_t rw_type = 0;
...
static ssize_t rtp_file_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
...
static ssize_t rtp_file_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	sih_haptic_t *sih_haptic = NULL;
...
static ssize_t main_loop_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
...
static ssize_t seq_gain_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	ssize_t len = 0;
	sih_haptic_t *sih_haptic = NULL;
...
static ssize_t seq_gain_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	sih_haptic_t *sih_haptic = NULL;
	unsigned int databuf[2] = { 0, 0 };
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		sih_haptic->hp_func->set_ram_seq_gain(
			sih_haptic, (uint8_t)databuf[0],
			sih_haptic->ram.gain[(uint8_t)databuf[0]]);
		mutex_unlock(&sih_haptic->lock);
	}
...
static ssize_t pwm_rate_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	ssize_t len = 0;
	sih_haptic_t *sih_haptic = NULL;
...
static ssize_t pwm_rate_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t count)
{
	sih_haptic_t *sih_haptic = NULL;
	uint32_t databuf[2] = { 0, 0 };
	...
	mutex_lock(&sih_haptic->lock);
	sih_haptic->hp_func->set_pwm_rate(sih_haptic, (uint8_t)databuf[0],
					  (uint8_t)databuf[1]);
	mutex_unlock(&sih_haptic->lock);
				  ...
				  const char *buf, size_t count)
{
	...
	mutex_lock(&sih_haptic->lock);
	sih_haptic->hp_func->set_detect_state(sih_haptic, databuf[0],
					      databuf[1]);
	mutex_unlock(&sih_haptic->lock);
...
static ssize_t detect_f0_show(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	sih_haptic_t *sih_haptic = NULL;
	ssize_t len = 0;
...
static DEVICE_ATTR(f0, S_IWUSR | S_IRUGO, tracking_f0_show, tracking_f0_store);
...
static DEVICE_ATTR(cont_go, S_IWUSR | S_IRUGO, NULL, cont_go_store);
static DEVICE_ATTR(cont_seq0, S_IWUSR | S_IRUGO, cont_seq0_show,
		   cont_seq0_store);
static DEVICE_ATTR(cont_seq1, S_IWUSR | S_IRUGO, cont_seq1_show,
		   cont_seq1_store);
static DEVICE_ATTR(cont_seq2, S_IWUSR | S_IRUGO, cont_seq2_show,
		   cont_seq2_store);
static DEVICE_ATTR(cont_asmooth, S_IWUSR | S_IRUGO, cont_asmooth_show,
		   cont_asmooth_store);
static DEVICE_ATTR(cont_th_len, S_IWUSR | S_IRUGO, cont_th_len_show,
		   cont_th_len_store);
static DEVICE_ATTR(cont_th_num, S_IWUSR | S_IRUGO, cont_th_num_show,
		   cont_th_num_store);
static DEVICE_ATTR(cont_ampli, S_IWUSR | S_IRUGO, cont_ampli_show,
		   cont_ampli_store);
static DEVICE_ATTR(ram, S_IWUSR | S_IRUGO, NULL, ram_store);
static DEVICE_ATTR(effect_id, S_IWUSR | S_IRUGO, effect_id_show,
		   effect_id_store);
...
static DEVICE_ATTR(f0_value, S_IRUGO, f0_value_show, NULL);
static DEVICE_ATTR(tracking_f0, S_IWUSR | S_IRUGO, tracking_f0_show, NULL);
static DEVICE_ATTR(detect_f0, S_IWUSR | S_IRUGO, detect_f0_show, NULL);
...
static DEVICE_ATTR(auto_pvdd, S_IWUSR | S_IRUGO, auto_pvdd_show,
		   auto_pvdd_store);
...
static DEVICE_ATTR(trig_para, S_IWUSR | S_IRUGO, trig_para_show,
		   trig_para_store);
static DEVICE_ATTR(low_power, S_IWUSR | S_IRUGO, low_power_show,
		   low_power_store);
static DEVICE_ATTR(activate, S_IWUSR | S_IRUGO, activate_show, activate_store);
static DEVICE_ATTR(drv_vboost, S_IWUSR | S_IRUGO, drv_vboost_show,
		   drv_vboost_store);
static DEVICE_ATTR(burst_rw, S_IWUSR | S_IRUGO, burst_rw_show, burst_rw_store);
static DEVICE_ATTR(rtp_file, S_IWUSR | S_IRUGO, rtp_file_show, rtp_file_store);
static DEVICE_ATTR(main_loop, S_IWUSR | S_IRUGO, main_loop_show,
		   main_loop_store);
static DEVICE_ATTR(seq_gain, S_IWUSR | S_IRUGO, seq_gain_show, seq_gain_store);
static DEVICE_ATTR(pwm_rate, S_IWUSR | S_IRUGO, pwm_rate_show, pwm_rate_store);
static DEVICE_ATTR(brake_ctrl, S_IWUSR | S_IRUGO, brake_ctrl_show,
		   brake_ctrl_store);
static DEVICE_ATTR(detect_state, S_IWUSR | S_IRUGO, detect_state_show,
		   detect_state_store);
static DEVICE_ATTR(custom_wave, S_IWUSR | S_IRUGO, custom_wave_show,
		   custom_wave_store);
...
{
	sih_haptic_t *sih_haptic =
		container_of(work, sih_haptic_t, ram.ram_work);

	hp_info("%s:enter!\n", __func__);
	hp_info("%s:play_mode = %d, effect_id = %d, state = %d, duration = %d\n",
		__func__, sih_haptic->ram.action_mode,
		sih_haptic->chip_ipara.effect_id, sih_haptic->chip_ipara.state,
		sih_haptic->chip_ipara.duration);
	if (sih_haptic->chip_ipara.effect_id >
	    sih_haptic->chip_ipara.effect_id_boundary) {
		hp_err("%s: effect id = %d, return.", __func__,
		       sih_haptic->chip_ipara.effect_id);
		return;
	}
	mutex_lock(&sih_haptic->lock);
	...
	if (sih_haptic->chip_ipara.state == SIH_ACTIVE_MODE) {
		...
		case SIH_RAM_MODE:
			sih_haptic->hp_func->set_gain(
				sih_haptic, sih_haptic->chip_ipara.gain);
...
static void rtp_work_func(struct work_struct *work)
{
	...
	const struct firmware *rtp_file;
	sih_haptic_t *sih_haptic =
		container_of(work, sih_haptic_t, rtp.rtp_work);
	...
	if (sih_haptic->chip_ipara.is_custom_wave == 1 &&
	    sih_haptic->chip_ipara.state) {
		hp_info("%s:buffer size %d, availbe size %d\n", __func__,
			sih_haptic->ram.base_addr >> 2, get_rb_avalible_size());
	...
	if (sih_haptic->chip_ipara.state) {
		pm_stay_awake(sih_haptic->dev);
		sih_effect_strength_gain_adapter(sih_haptic);
		sih_haptic->hp_func->set_gain(sih_haptic,
					      sih_haptic->chip_ipara.gain);
		sih_haptic->rtp.rtp_init = false;
		if (sih_haptic->chip_ipara.is_custom_wave == 0) {
			...
			if (ret < 0) {
				hp_err("%s:failed to read %s\n", __func__,
				       awinic_rtp_name[sih_haptic->rtp
							       .rtp_file_num]);
				sih_chip_state_recovery(sih_haptic);
				pm_relax(sih_haptic->dev);
				mutex_unlock(&sih_haptic->lock);
			...
			sih_vfree_container(sih_haptic,
					    sih_haptic->rtp.rtp_cont);
			sih_haptic->rtp.rtp_cont =
				vmalloc(rtp_file->size + sizeof(int));
			if (!sih_haptic->rtp.rtp_cont) {
				release_firmware(rtp_file);
				hp_err("%s:error allocating memory\n",
				       __func__);
			...
			sih_haptic->rtp.rtp_cont->len = rtp_file->size;
			memcpy(sih_haptic->rtp.rtp_cont->data, rtp_file->data,
			       rtp_file->size);
			release_firmware(rtp_file);
			hp_info("%s:rtp len is %d\n", __func__,
				sih_haptic->rtp.rtp_cont->len);
		} else {
			sih_vfree_container(sih_haptic,
					    sih_haptic->rtp.rtp_cont);
			sih_haptic->rtp.rtp_cont = NULL;
			if (sih_haptic->ram.base_addr != 0) {
				sih_haptic->rtp.rtp_cont = vmalloc(
					(sih_haptic->ram.base_addr >> 2) +
					sizeof(int));
			} else {
				hp_err("%s:ram update not done yet, return!",
				       __func__);
			}
			if (!sih_haptic->rtp.rtp_cont) {
				hp_err("%s:error allocating memory\n",
		...
		sih_haptic->rtp.rtp_init = true;
		sih_haptic->hp_func->check_detect_state(sih_haptic,
							SIH_RTP_MODE);
		sih_haptic->hp_func->set_play_mode(sih_haptic, SIH_RTP_MODE);
		sih_haptic->hp_func->play_go(sih_haptic, true);
		usleep_range(2000, 2500);
		while (cnt--) {
			if (sih_haptic->hp_func->if_chip_is_mode(
				    sih_haptic, SIH_RTP_MODE)) {
				rtp_work_flag = true;
				hp_info("%s:rtp go!\n", __func__);
				break;
			}
...
static int sih_i2c_probe(struct i2c_client *i2c)
{
	...
	/* registers regmap */
	sih_haptic->regmapp.regmapping =
		haptic_regmap_init(i2c, sih_haptic->regmapp.config);
	...
	if (ret < 0) {
		hp_err("%s register input device failed, ret=%d\n", __func__,
		       ret);
		XM_HAP_REGISTER_EXCEPTION("Device register",
	...
	sih_haptic->work_queue =
		create_singlethread_workqueue("sih_haptic_work_queue");
...
static const struct i2c_device_id sih_i2c_id[] = { { SIH_HAPTIC_NAME_688X, 0 },
						   {} };
