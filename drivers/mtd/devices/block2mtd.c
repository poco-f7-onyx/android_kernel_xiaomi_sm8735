					...
					size_t label, int timeout)
{
	...
	pr_info("mtd%d: [%s] erase_size = %dKiB [%d]\n", dev->mtd.index,
		dev->mtd.name + strlen("block2mtd: "), dev->mtd.erasesize >> 10,
		dev->mtd.erasesize);
	return dev;
...
err_free_block2mtd:
	...
	if (is_first_init == 1 && label == 1 && block2mtd_device_name != NULL) {
		is_first_init = 0;
		last_init_info =
			kzalloc(sizeof(struct device_init_info), GFP_KERNEL);
		...
		retry_thread = kthread_run(retry_add_device_thread_func, NULL,
					   "block2mtd_retry");
	}
	return NULL;
}
...
static int retry_add_device_thread_func(void *data)
{
	...
	while (retry_count > 0 && last_init_info != NULL) {
		if (add_device(last_init_info->devname,
			       last_init_info->erase_size,
			       last_init_info->label,
			       last_init_info->timeout) != NULL) {
			pr_err("devname is %s, erase_size is %d, label is %zu, timeout is %d\n",
			       last_init_info->devname,
			       last_init_info->erase_size,
			       last_init_info->label, last_init_info->timeout);
...
static int block2mtd_setup2(const char *val)
{
	...
	if (label == 1) {
		...
		block2mtd_device_name =
			(char *)kzalloc(length * sizeof(char), GFP_KERNEL);
		if (block2mtd_device_name == NULL) {
