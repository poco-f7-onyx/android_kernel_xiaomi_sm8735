...
MODULE_PARM_DESC(boot_index, "boot_index (default 0)");
			       ...
			       int nextpage)
{
	...
	ret_len = snprintf(
		...
		log_type[type], BootMode, ts.tm_year + 1900, ts.tm_mon + 1,
...
static void mtdoops_add_pmsg_head(char *oops_buf, enum mtdoops_log_type type)
{
	...
	ret_len = snprintf(
		str_buf, 80,
		"\n```\n#### LOG TYPE:%s\n#####%04ld-%02d-%02d %02d:%02d:%02d\n```\n",
		log_type[type], ts.tm_year + 1900, ts.tm_mon + 1, ts.tm_mday,
			    ...
			    enum mtd_dump_reason reason)
{
	...
	if (reason == KMSG_DUMP_SHUTDOWN || reason == KMSG_DUMP_EMERG)
		return;
