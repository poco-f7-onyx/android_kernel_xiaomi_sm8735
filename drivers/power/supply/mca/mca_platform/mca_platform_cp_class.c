...
int platform_class_cp_set_adjustadble_timeout(unsigned int role, int timeout)
{
	...
	if (platform_cp_ops_invalid(temp_data, cp_set_adjustadble_timeout))
		return -1;

	return temp_data->ops->cp_set_adjustadble_timeout(timeout, temp_data->data);
}
...
int platform_class_cp_get_battery_vout(unsigned int role, u32 *val)
{
	...
	if (platform_cp_ops_invalid(temp_data, cp_get_battery_vout))
		return -1;

	return temp_data->ops->cp_get_battery_vout(val, temp_data->data);
}
...
int platform_class_cp_enable_vbus_errorlo(unsigned int role, bool en)
{
	...
	if (platform_cp_ops_invalid(temp_data, cp_enable_vbus_errorlo))
		return -1;

	return temp_data->ops->cp_enable_vbus_errorlo(en, temp_data->data);
}
EXPORT_SYMBOL(platform_class_cp_enable_vbus_errorlo);
...
int platform_class_cp_set_manual_revchg_mode(unsigned int role, bool en)
{
	...
	if (platform_cp_ops_invalid(temp_data, cp_set_manual_revchg_mode))
		return -1;

	return temp_data->ops->cp_set_manual_revchg_mode(en, temp_data->data);
}
