					   ...
					   const char *effective_client)
{
	...
	ret = platform_class_buckchg_ops_set_wls_hiz(MAIN_BUCK_CHARGER,
						     effective_result);
	if (info->support_multi_buck)
		ret |= platform_class_buckchg_ops_set_wls_hiz(AUX_BUCK_CHARGER,
							      effective_result);
...
static int strategy_wireless_set_rx_vout(struct strategy_wireless_dev *info,
					 int vout)
{
	if (info->force_vout_6v)
		return platform_class_wireless_set_vout(WIRELESS_ROLE_MASTER,
							6000);
...
static int strategy_wireless_if_set_input_suspend(const char *user, char *value,
						  void *data)
{
	int temp_value = 0;
	struct strategy_wireless_dev *info = data;
...
static int strategy_wireless_if_set_chg_cur(const char *user, char *value,
					    void *data)
{
	int temp_value = 0;
	struct strategy_wireless_dev *info = data;
...
static int
strategy_wireless_get_compatible_info(struct strategy_wireless_dev *info)
{
	int i;
	...
	for (i = 0; i < info->uuid_adapter_info->uuid_para_size; i++) {
		if (info->proc_data.uuid_value ==
		    info->uuid_adapter_info[i].uuid)
			break;
	}
...
static int strategy_wireless_quiet_sts_func(struct strategy_wireless_dev *info)
{
	...
	if (info->quiet_sts) {
		mca_log_info(
			"enter wireless quiet charge mode, set tx to 0.%d speed\n",
			SMARTCHG_WLS_QUIET_FAN_SPEED);
		platform_class_wireless_set_tx_fan_speed(
			WIRELESS_ROLE_MASTER, SMARTCHG_WLS_QUIET_FAN_SPEED);
	} else {
		mca_log_info(
			"exit wireless quiet charge mode, set tx to 0.%d speed\n",
			SMARTCHG_WLS_NORMAL_FAN_SPEED);
		platform_class_wireless_set_tx_fan_speed(
			WIRELESS_ROLE_MASTER, SMARTCHG_WLS_NORMAL_FAN_SPEED);
	}
	...
	struct wls_adapter_power_cap *adapter_power)
{
	...
	memcpy(adapter_power, &info->proc_data.wireless_power,
	       sizeof(struct wls_adapter_power_cap));
...
int strategy_class_wireless_ops_set_parallel_charge(bool parallel_charge_flag)
{
	...
	ret = platform_class_wireless_set_parallel_charge(WIRELESS_ROLE_MASTER,
	...
	if (parallel_charge_flag) {
		if (info->proc_data.adapter_type >= ADAPTER_XIAOMI_PD_50W)
			schedule_delayed_work(&info->max_power_control_work,
			...
			if (info->proc_data.is_2_1_mode)
				...
				*cp_charger_mode = FORWARD_2_1_CHARGER_MODE;
				 ...
				 (info->uuid_adapter_info[num]
					  .compatible_info.car_mounted ||
				  ((info->uuid_adapter_info[num]
					    .compatible_info.sailboat_tx) &&
				   info->project_vendor ==
...
void strategy_class_wireless_op_get_rx_iout_limit(int *rx_iout_limit_ma)
{
	...
	case ADAPTER_XIAOMI_PD:
	case ADAPTER_ZIMI_CAR_POWER:
		if (info->proc_data.forward_charger_mode ==
	...
	case ADAPTER_XIAOMI_PD_100W:
		if (info->project_vendor == WLS_CHIP_VENDOR_FUDA1661 ||
			...
			if (info->proc_data.forward_charger_mode ==
			    FORWARD_2_1_CHARGER_MODE)
				*rx_iout_limit_ma =
					MCA_BASIC_WLS_RX_IOUT_MAX_50W_2_1;
			else
				*rx_iout_limit_ma =
					MCA_BASIC_WLS_RX_IOUT_MAX_80W_4_1;
		} else
			*rx_iout_limit_ma = MCA_BASIC_WLS_RX_IOUT_MAX_50W_4_1;
		break;
	default:
...
static void strategy_wireless_icl_setting(struct strategy_wireless_dev *info,
					  bool en, int mA)
{
...
static void strategy_wireless_fcc_setting(struct strategy_wireless_dev *info,
					  bool en, int mA)
{
	mca_log_info("en: %d, ma: %u\n", en, mA);
...
static void
strategy_wireless_qvalue_icl_setting(struct strategy_wireless_dev *info,
				     bool en, int mA)
{
	mca_log_info("en: %d, ma: %u\n", en, mA);
...
static void
strategy_wireless_qvalue_fcc_setting(struct strategy_wireless_dev *info,
				     bool en, int mA)
{
	mca_log_info("en: %d, ma: %u\n", en, mA);
...
static void
strategy_wireless_soc_fcc_setting(struct strategy_wireless_dev *info, bool en,
				  int mA)
{
	mca_log_info("en: %d, ma: %u\n", en, mA);
...
static void
strategy_wireless_soc_icl_setting(struct strategy_wireless_dev *info, bool en,
				  int mA)
{
	mca_log_info("en: %d, ma: %u\n", en, mA);
...
static void
strategy_wireless_audio_phone_icl_setting(struct strategy_wireless_dev *info,
					  bool en, int mA)
{
	mca_log_info("en: %d, ma: %u\n", en, mA);
...
static void
strategy_wireless_67w_fcc_setting(struct strategy_wireless_dev *info, bool en,
				  int mA)
{
	mca_log_info("en: %d, ma: %u\n", en, mA);
...
static void
strategy_wireless_67w_icl_setting(struct strategy_wireless_dev *info, bool en,
				  int mA)
{
	mca_log_info("en: %d, ma: %u\n", en, mA);
...
static void
strategy_wireless_stepper_pmic_icl(struct strategy_wireless_dev *info,
				   int start_icl, int end_icl, int step_ma,
				   int ms)
{
...
static void
strategy_wireless_add_trans_task_to_queue(struct strategy_wireless_dev *info,
...
static void strategy_wireless_get_qc_enable(struct strategy_wireless_dev *info,
					    bool *enable)
{
	if (!info->online) {
		*enable = false;
	...
	if (info->force_vout_6v || !info->proc_data.epp ||
	    !info->proc_data.fc_flag || !info->proc_data.qc_enable ||
	    !info->proc_data.pre_fastchg) {
		*enable = false;
		mca_log_err(
			"can't enable quick charge, force_vout_6v:%d  epp:%d  fc:%d  qc_enable:%d  "
			"pre_fastchg:%d\n",
	...
	if (info->proc_data.adapter_type == ADAPTER_XIAOMI_PD_40W &&
		...
		*enable = false;
		mca_log_err(
			"can't enable quick charge, magnetic adapter at F2\n");
		return;
	}
						  ...
						  const char *effective_client)
{
	...
	return platform_class_buckchg_ops_set_wls_input_curr_lmt(
		MAIN_BUCK_CHARGER, effective_result);
}
					   ...
					   const char *effective_client)
{
	...
	if (effective_result && !info->proc_data.epp &&
	    ((info->proc_data.adapter_type <= ADAPTER_DCP &&
	      info->proc_data.adapter_type > ADAPTER_NONE) ||
	     info->proc_data.adapter_type == ADAPTER_AUTH_FAILED ||
	     info->proc_data.adapter_type == ADAPTER_XIAOMI_PD))
		mca_vote(info->input_limit_voter, "wireless_thermal_bpp", true,
			 effective_result);
	else
		mca_vote(info->input_limit_voter, "wireless_thermal_bpp", false,
			 0);
					      ...
					      const char *effective_client)
{
	...
	if (effective_result && !info->proc_data.epp &&
	    info->proc_data.adapter_type == ADAPTER_QC2)
		mca_vote(info->input_limit_voter, "wireless_thermal_bppqc2",
			 true, effective_result);
	else
		mca_vote(info->input_limit_voter, "wireless_thermal_bppqc2",
			 false, 0);
					      ...
					      const char *effective_client)
{
	...
	if (effective_result && !info->proc_data.epp &&
	    info->proc_data.adapter_type == ADAPTER_QC3)
		mca_vote(info->input_limit_voter, "wireless_thermal_bppqc3",
			 true, effective_result);
	else
		mca_vote(info->input_limit_voter, "wireless_thermal_bppqc3",
			 false, 0);
					   ...
					   const char *effective_client)
{
	...
	if (effective_result && info->proc_data.epp &&
	    (info->proc_data.adapter_type == ADAPTER_PD ||
	     info->proc_data.adapter_type == ADAPTER_AUTH_FAILED))
		mca_vote(info->input_limit_voter, "wireless_thermal_epp", true,
			 effective_result);
	else
		mca_vote(info->input_limit_voter, "wireless_thermal_epp", false,
			 0);
					       ...
					       const char *effective_client)
{
	...
	if (effective_result &&
		...
		mca_vote(info->charge_limit_voter, "wireless_thermal_20w", true,
			 effective_result);
		mca_vote(info->sw_thermal_ichg_voter, "wireless_thermal", true,
			 effective_result);
	} else
		mca_vote(info->charge_limit_voter, "wireless_thermal_20w",
			 false, 0);
					       ...
					       const char *effective_client)
{
	...
	if (effective_result &&
	    info->proc_data.adapter_type == ADAPTER_XIAOMI_PD_40W &&
	    !info->proc_data.magnet_tx_flag) {
		mca_vote(info->charge_limit_voter, "wireless_thermal_30w", true,
			 effective_result);
		mca_vote(info->sw_thermal_ichg_voter, "wireless_thermal", true,
			 effective_result);
	} else
		mca_vote(info->charge_limit_voter, "wireless_thermal_30w",
			 false, 0);
					       ...
					       const char *effective_client)
{
	...
	if (effective_result &&
		...
		mca_vote(info->charge_limit_voter, "wireless_thermal_50w", true,
			 effective_result);
		mca_vote(info->sw_thermal_ichg_voter, "wireless_thermal", true,
			 effective_result);
	} else
		mca_vote(info->charge_limit_voter, "wireless_thermal_50w",
			 false, 0);
					       ...
					       const char *effective_client)
{
	...
	if (effective_result &&
	    info->proc_data.adapter_type >= ADAPTER_XIAOMI_PD_60W &&
	    info->proc_data.wireless_power.max_fcc > 9200) {
		mca_vote(info->charge_limit_voter, "wireless_thermal_80w", true,
			 effective_result);
		mca_vote(info->sw_thermal_ichg_voter, "wireless_thermal", true,
			 effective_result);
	} else
		mca_vote(info->charge_limit_voter, "wireless_thermal_80w",
			 false, 0);
...
static int
strategy_wireless_set_auth_voice_box_ichg(struct mca_votable *votable,
					  void *data, int effective_result,
					  const char *effective_client)
{
	struct strategy_wireless_dev *info = data;
	...
	if (effective_result &&
		...
		mca_vote(info->charge_limit_voter, "wireless_thermal_box", true,
			 effective_result);
		mca_vote(info->sw_thermal_ichg_voter, "wireless_thermal", true,
			 effective_result);
	} else
		mca_vote(info->charge_limit_voter, "wireless_thermal_box",
			 false, 0);
...
static int
strategy_wireless_set_auth_magnet_30w_ichg(struct mca_votable *votable,
					   void *data, int effective_result,
					   const char *effective_client)
{
	struct strategy_wireless_dev *info = data;
	...
	if (effective_result && info->proc_data.magnet_tx_flag &&
	    info->proc_data.adapter_type == ADAPTER_XIAOMI_PD_40W) {
		mca_vote(info->charge_limit_voter, "wireless_thermal_magnet",
			 true, effective_result);
		mca_vote(info->sw_thermal_ichg_voter, "wireless_thermal", true,
			 effective_result);
	} else
...
static int
strategy_wireless_set_fastchg_adapter(struct strategy_wireless_dev *info)
{
	int ret = 0;
	...
	case ADAPTER_PD:
		if (!info->proc_data.epp) {
			...
			ret = platform_class_wireless_set_adapter_voltage(
				WIRELESS_ROLE_MASTER, BPP_PLUS_VOUT);
			if (ret < 0)
				mca_log_info(
					"bpp+ set adapter voltage failed!!!\n");
		}
		break;
	case ADAPTER_XIAOMI_QC3:
	case ADAPTER_XIAOMI_PD:
...
static int strategy_wireless_get_max_power(struct strategy_wireless_dev *info)
{
	...
	if (max_power >= WLS_SSDEV_POWER_MAX_INVALID &&
	    max_power <= WLS_SSDEV_POWER_MAX_80W)
		return max_power;
	else
		return 0;
}
...
static int strategy_wireless_adapter_handle(struct strategy_wireless_dev *info)
{
	...
	if (!info->proc_data.fc_flag) {
		...
		case ADAPTER_QC2:
			mca_log_info(
				"[xiaomi]SDP/DCP/CDP/QC2 set iwls 750mA\n");
		...
		case ADAPTER_QC3:
			if (info->proc_data.epp) {
				if (info->proc_data.tx_max_power == 15) {
					...
					strategy_wireless_icl_setting(
						info, true, 1300);
					mca_log_info(
						"[xiaomi]FAIL/QC3/PD EPP set iwls 1300mA\n");
				} else {
					info->proc_data.pre_iwls = 850;
					strategy_wireless_icl_setting(
						info, true, 850);
					mca_log_info(
						"[xiaomi]FAIL/QC3/PD EPP set iwls 850mA\n");
				}
			} else {
				info->proc_data.pre_iwls = 750;
				...
				strategy_wireless_icl_setting(info, true, 750);
				mca_log_info(
					"[xiaomi]FAIL/QC3/PD BPP set iwls 750mA\n");
			}
			break;
		case ADAPTER_XIAOMI_QC3:
		case ADAPTER_XIAOMI_PD:
		...
		case ADAPTER_QC3:
			msleep(2000);
			strategy_wireless_stepper_pmic_icl(info, 800, 1100, 100,
							   20);
		...
		case ADAPTER_ZIMI_CAR_POWER:
			...
			mca_log_info(
				"[xiaomi]20W adapter set fcc as requested\n");
			break;
		case ADAPTER_XIAOMI_PD_40W:
		case ADAPTER_VOICE_BOX:
			info->proc_data.pre_vout = EPP_PLUS_VOUT;
			...
			mca_log_info(
				"[xiaomi]30W adapter set fcc as requested\n");
			break;
		case ADAPTER_XIAOMI_PD_60W:
		case ADAPTER_XIAOMI_PD_100W:
			info->proc_data.pre_vout = EPP_PLUS_VOUT;
			...
			info->proc_data.qc_type = ADP_ICON_TYPE_SUPER;
			mca_log_info(
				"[xiaomi]60W adapter set fcc as requested\n");
			break;
		case ADAPTER_XIAOMI_PD_50W:
			info->proc_data.pre_vout = EPP_PLUS_VOUT;
			strategy_wireless_fcc_setting(info, true, 9200);
			...
			info->proc_data.qc_type = ADP_ICON_TYPE_SUPER;
			mca_log_info(
				"[xiaomi]40~50W adapter set fcc as requested\n");
			info->proc_data.qc_enable = true;
			break;
		default:
			mca_log_info("[xiaomi]other adapter type\n");
	...
	schedule_delayed_work(&info->wireless_loop_work,
...
static void
strategy_wireless_update_soc_to_tx(struct strategy_wireless_dev *info)
{
	...
	(void)platform_class_wireless_send_transparent_data(
		WIRELESS_ROLE_MASTER, data, data_length);

	schedule_delayed_work(&info->mutex_unlock_work, msecs_to_jiffies(2000));
}

static void
strategy_wireless_get_target_vout_iwls(struct strategy_wireless_dev *info)
{
	switch (info->proc_data.adapter_type) {
	case ADAPTER_QC2:
...
static int strategy_wireless_get_chgr_stage(struct strategy_wireless_dev *info,
					    enum XM_WLS_CHGR_STAGE *chg_stage)
{
	bool charging_done;
...
static void
strategy_wireless_get_batt_chgr_status(struct strategy_wireless_dev *info,
				       int *chg_status)
{
	int rc = 0;
	...
	rc = power_supply_get_property(info->batt_psy, POWER_SUPPLY_PROP_STATUS,
				       &pval);
	if (rc < 0)
...
static int
strategy_wireless_send_frequency_request(struct strategy_wireless_dev *info,
					 u8 frequence_khz)
{
	...
	if (frequence_khz < SUPER_TX_FREQUENCY_MIN_KHZ ||
	    frequence_khz > SUPER_TX_FREQUENCY_MAX_KHZ)
		return -EINVAL;

	info->mutex_lock_sts = true;
	send_value[3] = frequence_khz;

	ret = platform_class_wireless_send_transparent_data(
		WIRELESS_ROLE_MASTER, send_value, ARRAY_SIZE(send_value));
	mca_log_info("%d\n", frequence_khz);
	schedule_delayed_work(&info->mutex_unlock_work, msecs_to_jiffies(2000));
...
static int
strategy_wireless_send_vout_range_request(struct strategy_wireless_dev *info,
					  int max_voltage_mv)
{
	...
	if (max_voltage_mv < SUPER_TX_VOUT_MIN_MV ||
	    max_voltage_mv > SUPER_TX_VOUT_MAX_MV)
		return -EINVAL;
...
strategy_wireless_transparent_success(struct strategy_wireless_dev *info)
{
	...
	case TX_CMD_TYPE_FREQUENCE:
		...
		if (info->support_mode == 2)
			...
			strategy_wireless_add_trans_task_to_queue(
				info, TRANS_DATA_FLAG_VOUT_RANGE,
				info->proc_data.tx_voltage);
...
static int
strategy_wireless_cmd_type_f5_func(struct strategy_wireless_dev *info, bool en)
{
	int ret = 0;
	...
	if (!en) {
		if (info->proc_data.chgr_stage == WLS_FULL_MODE)
			ret = strategy_wireless_set_rx_vout(info,
							    BPP_DEFAULT_VOUT);
		else
			ret = strategy_wireless_set_rx_vout(info,
							    EPP_DEFAULT_VOUT);
...
static int strategy_wireless_help_fcc_change(struct strategy_wireless_dev *info,
					     u8 *receive_data)
{
	int ret = 0;
	int i;
	...
	if ((receive_data[2] == info->proc_data.current_for_adapter_cmd) ||
	    (receive_data[2] < ADAPTER_CMD_TYPE_F0 ||
	     receive_data[2] > ADAPTER_CMD_TYPE_F5))
		return ret;
	...
	force_6v_result =
		mca_get_client_vote(info->force_vout_6v_voter, "cmd_type");
	...
	case ADAPTER_CMD_TYPE_F5:
		...
		if (icl_setting)
			strategy_wireless_67w_icl_setting(info, true,
							  icl_setting);
		if (fcc_setting)
			strategy_wireless_67w_fcc_setting(info, true,
							  fcc_setting);
		break;
	default:
		if (force_6v_result)
...
static int
strategy_wireless_transparent_fail(struct strategy_wireless_dev *info)
{
	int ret = 0;
	if (info->proc_data.set_tx_voltage_cnt++ < TRANS_FAIL_RETRY_MAX_CNT) {
		...
		case TX_CMD_TYPE_FREQUENCE:
			strategy_wireless_add_trans_task_to_queue(
				info, TRANS_DATA_FLAG_FREQUENCE,
				info->proc_data.tx_frequency);
			mca_log_info("frequency_range failed\n");
			break;
		case TX_CMD_TYPE_VOLTAGE:
			if (info->proc_data.adapter_type >
		...
		info->proc_data.current_for_tx_cmd = TX_CMD_TYPE_NONE;
		if ((!info->proc_data.first_drawload) &&
		    (!info->proc_data.is_4_1_mode)) {
			info->proc_data.is_2_1_mode = true;
			...
			schedule_delayed_work(
				&info->update_wireless_thermal_work,
				msecs_to_jiffies(0));
...
static void
strategy_wireless_epp_plate_tx_work(struct strategy_wireless_dev *info)
{
	...
	if (info->proc_data.target_vout != info->proc_data.pre_vout) {
		mca_log_info("epp plate, set new vout: %u, pre vout: %u\n",
			     info->proc_data.target_vout,
			     info->proc_data.pre_vout);

		if (info->proc_data.target_vout == EPP_PLUS_VOUT) {
			strategy_wireless_set_rx_vout(
				info, info->proc_data.target_vout);
			info->proc_data.qc_enable = true;
		} else if (info->proc_data.target_vout == EPP_DEFAULT_VOUT) {
			(void)platform_class_wireless_get_vout(
				WIRELESS_ROLE_MASTER, &vout);
	...
	if ((info->proc_data.target_iwls != info->proc_data.pre_iwls) ||
		...
		mca_log_info("epp plate, set new iwls: %u, pre iwls: %u\n",
			     info->proc_data.target_iwls,
			     info->proc_data.pre_iwls);
		strategy_wireless_icl_setting(info, true,
					      info->proc_data.target_iwls);
...
static void
strategy_wireless_epp_train_tx_work(struct strategy_wireless_dev *info)
{
	bool vout_change = false;
...
static void
strategy_wireless_qc2_adapter_work(struct strategy_wireless_dev *info)
{
	...
	if (info->proc_data.target_vout != info->proc_data.pre_vout) {
		mca_log_info("qc2 adapter, set new vout: %u, pre vout: %u\n",
			     info->proc_data.target_vout,
			     info->proc_data.pre_vout);
		strategy_wireless_set_rx_vout(info,
					      info->proc_data.target_vout);
		info->proc_data.pre_vout = info->proc_data.target_vout;
	}
...
static void
strategy_wireless_epp_standard_work(struct strategy_wireless_dev *info)
{
...
static void strategy_wireless_epp_plus_work(struct strategy_wireless_dev *info)
{
	...
	if (info->proc_data.chgr_stage == WLS_FULL_MODE) {
		...
		if (info->proc_data.target_iwls != info->proc_data.pre_iwls)
			strategy_wireless_icl_setting(
				info, true, info->proc_data.target_iwls);
	}
	...
	if (info->proc_data.chgr_stage == WLS_RECHG_MODE) {
		...
		info->proc_data.target_iwls = 550;
		if (info->proc_data.target_iwls != info->proc_data.pre_iwls)
			strategy_wireless_stepper_pmic_icl(
				info, info->proc_data.pre_iwls,
				info->proc_data.target_iwls, 100, 50);
	}
	...
	if (info->proc_data.batt_soc >= 70 &&
	    info->proc_data.high_soc_fcc_set == false) {
...
static void
strategy_wireless_epp_musical_box_work(struct strategy_wireless_dev *info)
{
	bool vout_change = false;
	int vout = 0, battery_temp = 0;
	(void)strategy_class_fg_ops_get_temperature(&battery_temp);
	...
	if (info->proc_data.target_vout != info->proc_data.pre_vout) {
		mca_log_info(
			"epp musical box, set new vout: %u, pre vout: %u\n",
	...
	if ((info->proc_data.target_iwls != info->proc_data.pre_iwls) ||
	    vout_change) {
		mca_log_info(
			"epp musical box, set new iwls: %u, pre iwls: %u\n",
...
static int
strategy_wireless_charge_loop_work(struct strategy_wireless_dev *info)
{
	int num;
	...
	if ((num >= 0 && num < EXTERNAL_ADAPTER_DEFAULT) &&
		...
		if (info->uuid_adapter_info[num].compatible_info.plate_tx) {
			...
			return 0;
		} else if (info->uuid_adapter_info[num]
				   .compatible_info.train_tx) {
...
static void
strategy_wireless_get_charging_info(struct strategy_wireless_dev *info)
{
	...
	(void)mca_strategy_func_get_status(STRATEGY_FUNC_TYPE_QUICK_WIRELESS,
					   ...
					   &enable_quickchg);
	if (info->proc_data.epp && info->proc_data.fc_flag &&
	    (!info->proc_data.start_vout_check_flag)) {
		++info->proc_data.start_wls_loop_cnt;
		if (enable_quickchg)
			info->proc_data.start_vout_check_flag = true;
		else if (info->proc_data.start_wls_loop_cnt >=
			 START_WLS_LOOP_CHECK_CNT) {
			info->proc_data.start_vout_check_flag = true;
			mca_log_info(
				"after epp fastcharge, no enter quick_chg mode, set vout 11V\n");
			if (!info->proc_data.magnet_tx_flag ||
	...
	mca_log_err(
		"wireless loop: vout:%d, vrect:%d, iout:%d, iwls:%d, vbat_cell_mv:%d, soc:%d, tx_adapter:%d\n",
					 ...
					 bool enable)
{
	...
	ret = platform_class_buckchg_ops_set_wls_vdd_flag(
		MAIN_BUCK_CHARGER, info->proc_data.boost_wireless_vdd);
	if (ret) {
		mca_log_err("set wireless vdd boost fail, retry\n");
		schedule_delayed_work(&info->set_vdd_flag_work,
...
static int
strategy_wireless_phone_sts_change(struct strategy_wireless_dev *info)
{
	int ret = 0;
	...
	if (info->online && info->proc_data.magnet_tx_flag) {
		if (info->audio_phone_sts) {
			...
			if (ret < 0)
				mca_log_info(
					"work around for audio phone state set rx voltage failed!!!\n");
			strategy_wireless_audio_phone_icl_setting(
				info, true, info->phone_icl);
		} else if (info->proc_data.chgr_stage != WLS_FULL_MODE) {
			ret = strategy_wireless_set_rx_vout(info, 11000);
			if (ret < 0)
				mca_log_info(
					"work around for audio phone state reset rx voltage failed!!!\n");
			strategy_wireless_audio_phone_icl_setting(info, false,
								  0);
		...
		if (info->audio_phone_sts) {
			...
			if (ret < 0)
				mca_log_info(
					"work around for audio phone state set rx voltage failed!!!\n");
			strategy_wireless_audio_phone_icl_setting(
				info, true, info->offstd_phone_icl);
		} else {
			if (info->proc_data.epp)
				ret = strategy_wireless_set_rx_vout(info,
								    11000);
...
static void
strategy_wireless_update_quick_charge_type(struct strategy_wireless_dev *info)
{
	...
	info->proc_data.wireless_power.max_power =
		strategy_wireless_get_max_power(info);
	if (info->proc_data.wireless_power.max_power >=
	    MCA_WLS_CHG_SUPER_CHG_POWER)
		info->proc_data.qc_type = ADP_ICON_TYPE_SUPER;
	else if (info->proc_data.wireless_power.max_power >=
		 MCA_WLS_CHG_FLASH_CHG_POWER)
		info->proc_data.qc_type = ADP_ICON_TYPE_FLASH;
	else
		info->proc_data.qc_type = ADP_ICON_TYPE_NORMAL;
}
...
static void
strategy_wireless_pre_authen_process(struct strategy_wireless_dev *info)
{
	int ret = 0;
	int len = 0;
	int tx_adapter = 0;
	...
	ret = platform_class_wireless_get_tx_adapter_by_i2c(
		WIRELESS_ROLE_MASTER, &tx_adapter);
	if (ret) {
	...
	len = snprintf(event, MCA_EVENT_NOTIFY_SIZE,
		       "POWER_SUPPLY_TX_ADAPTER=%d",
		       info->proc_data.adapter_type);
	...
	len = snprintf(event, MCA_EVENT_NOTIFY_SIZE,
		       "POWER_SUPPLY_QUICK_CHARGE_TYPE=%d",
		       info->proc_data.qc_type);
	event_data.event = event;
	event_data.event_len = len;
	mca_event_report_uevent(&event_data);
}
...
static void strategy_wireless_power_good_on(struct strategy_wireless_dev *info)
{
	...
	if (ret) {
		...
		if (ret && power_good_flag) {
			mca_charge_mievent_report(CHARGE_DFX_WLS_RX_IIC_ERR,
						  NULL, 0);
	...
	wls_input_suspend =
		mca_get_effective_result(info->wls_input_suspend_voter);
	if (!wls_input_suspend)
		platform_class_buckchg_ops_set_wls_hiz(MAIN_BUCK_CHARGER,
						       false);
	...
	if (info->proc_data.batt_soc >= 70) {
		info->proc_data.high_soc_fcc_set = true;
		strategy_wireless_soc_fcc_setting(info, true,
						  6000); //temp, depend on dts
	}

	//check fw_version
	platform_class_wireless_check_firmware_state(WIRELESS_ROLE_MASTER,
						     &fw_update);
	if (fw_update)
		schedule_delayed_work(&info->wls_fw_state_work,
				      msecs_to_jiffies(10000));
	else {
		mca_wireless_rev_set_firmware_state(FIRMWARE_NO_UPDATE);
		len = snprintf(event, MCA_EVENT_NOTIFY_SIZE,
			       "POWER_SUPPLY_WLS_FW_STATE=%d",
			       FIRMWARE_NO_UPDATE);
		event_data.event = event;
		event_data.event_len = len;
		mca_event_report_uevent(&event_data);
...
static void
strategy_wireless_reset_charge_para(struct strategy_wireless_dev *info)
{
...
static void
strategy_wireless_process_online_change(int value,
...
static void strategy_wireless_chg_handler(struct strategy_wireless_dev *info)
{
	...
	case RX_INT_POWER_ON:
		...
		ret = platform_class_wireless_get_rx_power_mode(
	...
	case RX_INT_LDO_ON:
		...
		cancel_delayed_work_sync(&info->rx_fastcharge_work);
		platform_class_buckchg_ops_set_aicl_enable(MAIN_BUCK_CHARGER,
							   false);

		ret = platform_class_wireless_get_ss_voltage(
			WIRELESS_ROLE_MASTER, &info->proc_data.ss_voltage);
		if (ret)
		...
		schedule_delayed_work(&info->update_wireless_thermal_work,
				      msecs_to_jiffies(0));
		ret = platform_class_wireless_get_tx_max_power(
			WIRELESS_ROLE_MASTER, &info->proc_data.tx_max_power);
		if (ret)
			mca_log_info("get tx max power failed\n");

		if (info->proc_data.epp) {
			strategy_wireless_icl_setting(info, true, 200);
			schedule_delayed_work(&info->wls_drawload_work,
					      msecs_to_jiffies(300));
		} else {
			if (info->project_vendor == WLS_CHIP_VENDOR_FUDA1651)
				strategy_wireless_stepper_pmic_icl(
					info, 250, 750, 100, 20);
			else if (info->project_vendor ==
	...
	case RX_INT_AUTHEN_FINISH:
		...
		platform_class_wireless_get_auth_value(
			WIRELESS_ROLE_MASTER, &info->proc_data.auth_data);
		platform_class_wireless_get_tx_uuid(WIRELESS_ROLE_MASTER,
						    info->proc_data.uuid);
		info->proc_data.uuid_value |= (info->proc_data.uuid[0] << 24 |
					       info->proc_data.uuid[1] << 16 |
					       info->proc_data.uuid[2] << 8 |
					       info->proc_data.uuid[3]);
		mca_log_err("uuid_value is 0x%08x\n",
			    info->proc_data.uuid_value);
		platform_class_wireless_get_tx_adapter(
			WIRELESS_ROLE_MASTER, &info->proc_data.adapter_type);

		if (info->proc_data.auth_data != AUTH_STATUS_FAILED) {
			if (info->proc_data.auth_data >= AUTH_STATUS_UUID_OK) {
				platform_class_wireless_get_debug_fod_type(
					WIRELESS_ROLE_MASTER, &type);
				if (type != WLS_DEBUG_SET_FOD_NONE)
					platform_class_wireless_set_debug_fod_params(
						WIRELESS_ROLE_MASTER);
				else
					platform_class_wireless_set_fod_params(
						WIRELESS_ROLE_MASTER,
						info->proc_data.auth_data);
			}

			if (!info->proc_data.epp)
				mca_log_info("bpp tx hw id: 0x%x, 0x%x\n",
					     info->proc_data.uuid[0],
					     info->proc_data.uuid[1]);
			else
				num = strategy_wireless_get_compatible_info(
					info);

			if (info->proc_data.uuid[2] & 0x80)
				info->proc_data.magnet_tx_flag = true;
			mca_log_info("uuid2: 0x%x, magnet_tx_flag: %d\n",
				     info->proc_data.uuid[2],
				     info->proc_data.magnet_tx_flag);
			...
			if (num >= 0 && num < EXTERNAL_ADAPTER_DEFAULT) {
				if (info->uuid_adapter_info[num]
					...
					len = snprintf(
						...
						"POWER_SUPPLY_WLS_CAR_ADAPTER=%d",
				...
				if (info->uuid_adapter_info[num]
					    .compatible_info.musical_box)
					info->proc_data.adapter_type =
						ADAPTER_VOICE_BOX;

				if (info->project_vendor ==
					    WLS_CHIP_VENDOR_FUDA1665 &&
				    info->uuid_adapter_info[num]
					    .compatible_info
			...
			if (num >= 0 && num < EXTERNAL_ADAPTER_DEFAULT) {
				if ((info->uuid_adapter_info[num]
					...
					strategy_wireless_add_trans_task_to_queue(
						info, TRANS_DATA_FLAG_QVALUE,
						0);
				}
			}

			strategy_wireless_update_quick_charge_type(info);
			if (info->proc_data.adapter_type_first !=
			    info->proc_data.adapter_type) {
				len = snprintf(event, MCA_EVENT_NOTIFY_SIZE,
					       "POWER_SUPPLY_TX_ADAPTER=%d",
					       info->proc_data.adapter_type);
				...
				schedule_delayed_work(
					&info->report_soc_decimal_work,
					msecs_to_jiffies(0));
			}

			if (info->proc_data.adapter_type >= ADAPTER_XIAOMI_QC3)
				strategy_wireless_add_trans_task_to_queue(
					info, TRANS_DATA_FLAG_SOC, 0);

			strategy_wireless_add_trans_task_to_queue(
				info, TRANS_DATA_FLAG_FAN_SPEED, 0);

			interval = info->head_cnt > 0 ? info->head_cnt * 500 :
							500;
			if (info->proc_data.adapter_type >=
			    ADAPTER_XIAOMI_PD_50W)
				schedule_delayed_work(
					&info->renegociation_work,
					msecs_to_jiffies(interval));
			else {
				strategy_wireless_adapter_handle(info);
				schedule_delayed_work(
					&info->rx_fastcharge_work,
					msecs_to_jiffies(interval));
			}
		} else {
			...
			if (info->support_hall &&
			    info->proc_data.magnetic_case_flag)
	...
	case RX_INT_RENEGO_DONE:
		...
		cancel_delayed_work_sync(&info->renegociation_work);
		strategy_wireless_rx_fastcharge_work(
			&info->rx_fastcharge_work.work);
		break;
	case RX_INT_RENEGO_FAIL:
		mca_log_info("RX_INT_RENEGO_FAIL\n");
		cancel_delayed_work_sync(&info->renegociation_work);
		schedule_delayed_work(&info->renegociation_work,
				      msecs_to_jiffies(0));
		break;
	case RX_INT_FAST_CHARGE:
		mca_log_err("RX_INT_FAST_CHARGE\n");
		platform_class_wireless_get_rx_fastcharge_status(
			WIRELESS_ROLE_MASTER, &info->proc_data.fc_flag);
		...
		if (info->proc_data.fc_flag) {
			if (info->project_vendor == WLS_CHIP_VENDOR_FUDA1665 ||
				...
				if (info->proc_data.adapter_type <
					...
					info->proc_data.current_for_tx_cmd =
			...
			(void)platform_class_buckchg_ops_get_otg_boost_src(
				MAIN_BUCK_CHARGER, &info->otg_boost_src);
			if (info->proc_data.epp &&
			    info->otg_boost_src == info->wls_vdd_src) {
				strategy_wireless_enable_vdd(info, true);
				if (info->project_vendor ==
				    WLS_CHIP_VENDOR_SC96281)
					platform_class_wireless_enable_vsys_ctrl(
						WIRELESS_ROLE_MASTER, true);
			}
		} else if (info->proc_data.adapter_type >=
				   ADAPTER_XIAOMI_PD_40W &&
			   info->proc_data.set_fastcharge_vout_cnt <
				   FAST_CHARGE_RETRY_MAX_CNT)
			schedule_delayed_work(&info->rx_fastcharge_work,
					      msecs_to_jiffies(0));
		else {
			strategy_wireless_adapter_handle(info);
			schedule_delayed_work(
	...
	case RX_INT_TRANSPARENT_SUCCESS:
		platform_class_wireless_receive_transparent_data(
			WIRELESS_ROLE_MASTER, rcv_value, ARRAY_SIZE(rcv_value),
			&val_length);
		cancel_delayed_work_sync(&info->mutex_unlock_work);
		strategy_wireless_mutex_unlock_work(
			&info->mutex_unlock_work.work);
		...
		if (l_len == 1) {
			...
			case 0x28:
				...
				if (rcv_val == 0x3C)
					(void)strategy_wireless_transparent_success(
						info);
				else
					(void)strategy_wireless_transparent_fail(
						info);
				break;
			case 0x05:
				(void)strategy_wireless_help_fcc_change(
					info, rcv_value);
				break;
			}
		} else if (l_len == 4) {
			if (rcv_value[4] == 0x62) {
				...
				if (num >= 0 &&
					...
					if (rcv_value[5] <= tx_q2) {
						strategy_wireless_qvalue_fcc_setting(
							info, true,
							MCA_WLS_CHG_TX_QLIMIT_FCC_5W);
						strategy_wireless_qvalue_icl_setting(
							info, true,
							MCA_WLS_CHG_TX_QLIMIT_ICL_5W);
						mca_log_info(
							"Q value is smaller than Q2, set power to 5W\n");
						dfx_data[0] = tx_q2;
	...
	case RX_INT_OCP_OTP_ALARM:
		...
		schedule_delayed_work(&info->rx_alarm_work,
				      msecs_to_jiffies(500));
		break;
	case RX_INT_POWER_OFF:
		(void)platform_class_wireless_get_poweroff_err_code(
			WIRELESS_ROLE_MASTER, &err_code);
		mca_log_err("RX_INT_POWER_OFF, err_code:0x%02x\n", err_code);
		switch (err_code) {
		case 3:
			mca_charge_mievent_report(CHARGE_DFX_WLS_RX_OTP,
						  &info->rx_temp, 1);
			break;
		case 4:
			mca_charge_mievent_report(CHARGE_DFX_WLS_RX_OVP, NULL,
						  0);
			break;
		case 5:
			mca_charge_mievent_report(CHARGE_DFX_WLS_RX_OCP, NULL,
						  0);
			break;
		default:
			break;
...
#ifdef CONFIG_FACTORY_BUILD
		(void)platform_class_wireless_receive_test_cmd(
			WIRELESS_ROLE_MASTER, rcv_value, &val_length);
		mca_log_err("RX_INT_FACTORY_TEST, %d, 0x%x, 0x%x, 0x%x\n",
			    val_length, rcv_value[0], rcv_value[1],
			    rcv_value[2]);

		if (rcv_value[0] == FACTORY_TEST_CMD) {
			if (rcv_value[1] == FACTORY_TEST_CMD_REVERSE_REQ)
				info->wait_for_reverse_test = true;
			(void)platform_class_wireless_process_factory_cmd(
				WIRELESS_ROLE_MASTER, rcv_value[1]);
		}
#endif
		break;
	case RX_INT_ERR_CODE:
		(void)platform_class_wireless_get_rx_err_code(
			WIRELESS_ROLE_MASTER, &err_code);
...
static void
strategy_wireless_process_batt_btb_change(int value,
					  struct strategy_wireless_dev *info)
{
	mca_log_info("value: %d\n", value);
	if (value) {
		mca_vote(info->charge_limit_voter, "batt_miss", true,
			 MCA_WLS_CHG_BATT_MISS_FCC);
#ifndef CONFIG_FACTORY_BUILD
		mca_vote(info->vterm_voter, "batt_miss", true,
			 MCA_WLS_CHG_BATT_MISS_FV);
#endif
	} else {
		mca_vote(info->charge_limit_voter, "batt_miss", false,
			 MCA_WLS_CHG_CURRENT_DEFAULT_VALUE);
...
static void
strategy_wireless_process_soc_limit_change(int value,
					   struct strategy_wireless_dev *info)
{
	static u8 cur_step;
...
static void strategy_wireless_process_battery_health_change(
	struct strategy_wireless_dev *info)
{
	union power_supply_propval pval = {
		0,
	...
	power_supply_get_property(info->batt_psy, POWER_SUPPLY_PROP_HEALTH,
				  &pval);
...
static int strategy_wireless_process_event(int event, int value, void *data)
{
	...
	case MCA_EVENT_WIRELESS_DISCONNECT:
		qcchg_disable_voter = mca_find_votable("wls_quick_chg_disable");
		if (qcchg_disable_voter)
			mca_vote(qcchg_disable_voter, "basic_wls_set", true,
				 !value);
	...
	case MCA_EVENT_CHARGE_RESTORE:
		...
		if (if_qc_enable && (info->otg_boost_src == info->wls_vdd_src))
			...
			ret = platform_class_buckchg_ops_set_wls_vdd_flag(
				MAIN_BUCK_CHARGER,
				info->proc_data.boost_wireless_vdd);
			if (ret) {
				mca_log_err(
					"set wireless vdd boost fail, retry\n");
				schedule_delayed_work(&info->set_vdd_flag_work,
						      msecs_to_jiffies(500));
			}
		}
...
static void strategy_wireless_check_rx_alarm(struct strategy_wireless_dev *info,
					     ...
					     bool *otp_recovery)
{
	...
	if (info->project_vendor == WLS_CHIP_VENDOR_SC96281) {
		...
		*otp_recovery = (info->rx_temp <=
				 MCA_WLS_CHG_RX_MAX_TEMP_SC96281 -
					 MCA_WLS_CHG_RX_OTP_DELTA_TEMP);
	} else {
		*otp_flag = (info->rx_temp >= MCA_WLS_CHG_RX_MAX_TEMP);
		*otp_recovery = (info->rx_temp <=
				 MCA_WLS_CHG_RX_MAX_TEMP -
					 MCA_WLS_CHG_RX_OTP_DELTA_TEMP);
	}
}
...
static void strategy_wireless_start_renego(struct strategy_wireless_dev *info)
{
	...
	if (max_power > 0)
		(void)platform_class_wireless_do_renego(WIRELESS_ROLE_MASTER,
							max_power);
}

static int
strategy_wireless_send_tx_q_value_request(struct strategy_wireless_dev *info)
{
	int ret = 0;
	u8 send_value = 0;
	int num;
	...
	if (num >= 0 && num < EXTERNAL_ADAPTER_DEFAULT) {
		if (info->uuid_adapter_info[num]
			    .compatible_info.low_inductance_50w_tx)
			send_value = info->tx_q1[0];
		if (info->uuid_adapter_info[num]
			    .compatible_info.low_inductance_80w_tx)
			send_value = info->tx_q1[1];
	}
	ret = platform_class_wireless_send_tx_q_value(WIRELESS_ROLE_MASTER,
						      send_value);
...
static int
strategy_wireless_enable_fast_charge_mode(struct strategy_wireless_dev *info)
{
	int batt_temp, fastcharge_mode, effective_icl, effective_fcc;
	int ret = 0;
	int vbat_cell_mv = 4000, rx_vout_mv = 0, ffc_iterm = 0,
	    normal_vterm = 0;
	...
	if (info->proc_data.adapter_type > ADAPTER_AUTH_FAILED) {
		...
		effective_icl =
			mca_get_effective_result(info->input_limit_voter);
		effective_fcc =
			mca_get_effective_result(info->charge_limit_voter);

		(void)platform_class_wireless_get_vout(WIRELESS_ROLE_MASTER,
						       &rx_vout_mv);
		if (rx_vout_mv <= BPP_DEFAULT_VOUT ||
		    rx_vout_mv >= EPP_PLUS_VOUT)
		...
		strategy_wireless_get_chgr_stage(info,
						 &info->proc_data.chgr_stage);
		mca_log_info(
			"batt_temp:%d, fastcharge_mode:%d, ffc_iterm:%d, effective_icl:%d, effective_fcc:%d, vbat_cell_mv:%d, rx_vout_mv:%d\n",
			batt_temp, fastcharge_mode, ffc_iterm, effective_icl,
			effective_fcc, vbat_cell_mv, rx_vout_mv);
		...
		if (normal_vterm &&
		    (vbat_cell_mv >= normal_vterm -
					     info->smartchg_data.delta_fv -
					     CHARGE_SW_CV_VBAT_ALARM_DELTA) &&
		    (effective_fcc < ffc_iterm ||
		     effective_icl * rx_vout_mv / vbat_cell_mv < ffc_iterm)) {
			if (fastcharge_mode) {
				fastchg_cnt++;
				if (fastchg_cnt >
				    MCA_WLS_CHG_ENABLE_FASTCHG_CNT) {
					fastchg_cnt = 0;
					strategy_class_fg_set_fastcharge(false);
					mca_log_info(
						"vcell is high and max fcc is low, exit ffc mode to avoid vcell over voltage\n");
					return ret;
				}
				...
				fastchg_cnt = 0;
				mca_log_info(
					"vcell is high and max fcc is low, keep normal mode to avoid vcell over voltage\n");
				return ret;
			}
		} else {
			mca_log_info(
				"vcell is low or max_fcc is high, clean fastchg_cnt\n");
			fastchg_cnt = 0;
		}

		if (batt_temp < info->ffc_temp_low ||
		    batt_temp >= info->ffc_temp_high) {
			if (fastcharge_mode) {
				strategy_class_fg_set_fastcharge(false);
				mca_log_info(
					"temp is over, wireless buck charger disable fast charge mode\n");
				return ret;
			}
			mca_log_info(
				"temp is over, wireless buck charger keep normal charge mode\n");
			return ret;
		}

		if (normal_vterm &&
		    (vbat_cell_mv <
		     normal_vterm - info->smartchg_data.delta_fv -
			     2 * CHARGE_SW_CV_VBAT_ALARM_DELTA) &&
		    !fastcharge_mode) {
			strategy_class_fg_set_fastcharge(true);
			mca_log_info(
				"wireless buck charger enable fast charge mode\n");
		}
	}
	return ret;
}
...
static void strategy_wireless_monitor_work(struct work_struct *work)
{
	...
	if (!info->thermal_phone_flag)
		(void)mca_vote(info->input_limit_voter, "thermal_phone", false,
			       0);

	jeita_hot_result =
		mca_get_client_vote(info->chg_enable_voter, "jeita-hot");
	...
	if (!input_suspend && info->proc_data.chgr_stage != WLS_FULL_MODE) {
		(void)platform_class_buckchg_ops_get_chg_status(
			MAIN_BUCK_CHARGER, &chgr_stat);
	...
	if (!info->sw_cv_running &&
	    ...
	    vterm >= MCA_WLS_CHG_VTERM_LOW_TH &&
	    info->proc_data.vbat_cell_mv >=
...
static void strategy_wireless_find_voter_work(struct work_struct *work)
{
	...
	if (ret) {
		...
		schedule_delayed_work(&info->find_voter_work,
				      msecs_to_jiffies(500));
	}
}
...
static void strategy_wireless_rx_fastcharge_work(struct work_struct *work)
{
	...
	if (info->proc_data.set_fastcharge_vout_cnt <
		    FAST_CHARGE_RETRY_MAX_CNT &&
	    info->proc_data.adapter_type >= ADAPTER_XIAOMI_QC3) {
		schedule_delayed_work(&info->rx_fastcharge_work,
				      msecs_to_jiffies(3500));
		++info->proc_data.set_fastcharge_vout_cnt;
	} else if (info->proc_data.set_fastcharge_vout_cnt >=
		   FAST_CHARGE_RETRY_MAX_CNT) {
...
static void strategy_wireless_drawload_work(struct work_struct *work)
{
	...
	while (cnt++ <= 50) {
		(void)platform_class_buckchg_ops_get_wls_curr(MAIN_BUCK_CHARGER,
							      &iwls);
		(void)platform_class_wireless_get_vout(WIRELESS_ROLE_MASTER,
						       &rx_vout);
		(void)platform_class_wireless_get_iout(WIRELESS_ROLE_MASTER,
						       &rx_iout);
		(void)platform_class_buckchg_ops_get_chg_status(
			MAIN_BUCK_CHARGER, &chgr_stat);
		mca_log_info(
			"BPP drawload, iwls:%d, chgr_sts:%d, iout:%d, vout:%d\n",
			iwls, chgr_stat, rx_iout, rx_vout);
		if (chgr_stat == CHARGER_STATUS_FULLON ||
		    chgr_stat == CHARGER_STATUS_TAPER)
			break;
	...
	if (info->proc_data.epp) {
		if (info->proc_data.tx_max_power == 15)
			strategy_wireless_stepper_pmic_icl(info, 200, 1300, 100,
							   20);
		else
			strategy_wireless_stepper_pmic_icl(info, 200, 800, 100,
							   20);
	} else
...
static void strategy_wireless_rx_alarm_work(struct work_struct *work)
{
	...
	strategy_wireless_check_rx_alarm(info, &ocp_flag, &otp_flag,
					 &otp_recovery);
	mca_log_info("ocp_otp alarm ocp_flag:%d otp_flag:%d\n", ocp_flag,
		     otp_flag);
	if ((ocp_flag == false) && (otp_flag == false) &&
	    (otp_recovery == true)) {
		strategy_wireless_otp_fcc_setting(info, false, 0);
		return;
	}
	...
	if (otp_flag) {
		if (current_fcc_val - 500 > 0)
			strategy_wireless_otp_fcc_setting(
				info, true, (current_fcc_val - 500));
	}
	...
	if (info->project_vendor == WLS_CHIP_VENDOR_FUDA1661 &&
		...
		if (num < 0 || num >= EXTERNAL_ADAPTER_DEFAULT)
			return;
				 ...
				 .compatible_info.standard_tx) {
			mca_log_info("max_power_control\n");
			strategy_wireless_fcc_setting(
				info, true,
				MCA_WLS_CHG_REDUCE_FCC_VALUE_80W_MA);
		}
	} else if (info->project_vendor == WLS_CHIP_VENDOR_FUDA1651 ||
		   info->project_vendor == WLS_CHIP_VENDOR_SC96281) {
		mca_log_info("max_power_control\n");
		strategy_wireless_fcc_setting(
			info, true, MCA_WLS_CHG_REDUCE_FCC_VALUE_50W_MA);
	}
}
...
static void
strategy_wireless_update_wireless_thermal_work(struct work_struct *work)
{
	struct strategy_wireless_dev *info =
		container_of(work, struct strategy_wireless_dev,
			     update_wireless_thermal_work.work);
...
static void
strategy_wireless_process_trans_func(struct strategy_wireless_dev *info,
...
static int strategy_wireless_process_trans(struct strategy_wireless_dev *info)
{
	...
	while (!list_empty(&info->header) && !info->mutex_lock_sts) {
		spin_lock(&info->list_lock);
		list_for_each_entry_safe(cur_node, temp_node, &info->header,
					 lnode) {
			if (info->mutex_lock_sts)
				break;
...
static int strategy_wireless_parse_cmd_type_para(
	struct device_node *node, const char *name,
	struct adapter_cmd_type_info *cmd_type_info)
{
	...
	cmd_type_info->cmd_fx = kcalloc(
		len_temp, sizeof(struct adapter_cmd_type_fx), GFP_KERNEL);

	for (i = 0; i < len_temp; i++) {
		j = ADAPTER_CMD_TYPE_SETTING_MAX * i;
		cmd_type_info->cmd_fx[i].icl_setting =
			idata[j + ADAPTER_CMD_TYPE_SETTING_ICL];
		cmd_type_info->cmd_fx[i].fcc_setting =
			idata[j + ADAPTER_CMD_TYPE_SETTING_FCC];
...
static int strategy_wireless_parse_adapter_cmd_type_info(
	struct strategy_wireless_dev *info)
{
	...
	for (i = 0; i < array_len; i++) {
		if (mca_parse_dts_string_index(node, "adapter_cmd_type_para", i,
					       &tmp_string))
			return -1;
		...
		case ADAPTER_CMD_TYPE_FX:
			if (kstrtou8(tmp_string, 16,
				     &info->cmd_type_info[row].receive_data))
				return -1;
			break;
		case ADAPTER_CMD_TYPE_FX_PARA:
			if (strategy_wireless_parse_cmd_type_para(
				    node, tmp_string,
				    &info->cmd_type_info[row]))
				return -1;
			break;
		default:
			break;
...
static int strategy_wireless_parse_adapter_para(
	struct device_node *node, const char *name,
	struct wireless_compatible_info *compatible_info)
{
	...
	array_len =
		mca_parse_dts_count_strings(node, name, ADAPTER_PARA_MAX_GROUP,
...
static int
strategy_wireless_parse_adapter_info(struct strategy_wireless_dev *info)
{
	struct device_node *node = info->dev->of_node;
	int array_len, row, col, i;
	const char *tmp_string = NULL;
	...
	info->uuid_adapter_info->uuid_para_size =
		array_len / MCA_BASIC_WLS_UUID_PARA_MAX;

	for (i = 0; i < array_len; i++) {
		if (mca_parse_dts_string_index(node, "adapter_para", i,
					       &tmp_string))
			return -1;
		...
		case MCA_BASIC_WLS_UUID_NAME:
			if (kstrtoint(tmp_string, 16,
				      &info->uuid_adapter_info[row].uuid))
				return -1;
			break;
		case MCA_BASIC_WLS_UUID_PARA:
			if (strategy_wireless_parse_adapter_para(
				    node, tmp_string,
				    &info->uuid_adapter_info[row]
					     .compatible_info))
				return -1;
			break;
		default:
...
{
	struct strategy_wireless_dev *info =
		(struct strategy_wireless_dev *)data;
	...
	case STRATEGY_STATUS_TYPE_WLS_MAGNET_LIMIT:
		*cur_val = info->online && info->proc_data.magnet_tx_flag &&
			   info->proc_data.magnetic_case_flag;
		break;
	default:
		return -1;
...
static int strategy_wireless_parse_dt(struct strategy_wireless_dev *info)
{
	...
	(void)mca_parse_dts_u32(node, "offstd_phone_icl",
				&info->offstd_phone_icl, 200);
	(void)mca_parse_dts_u32(node, "phone_vol", &info->phone_vol, 6000);

	(void)mca_parse_dts_u32(node, "ffc_temp_low", &info->ffc_temp_low,
				ALLOW_FFC_TEMP_LOW_THR);
	(void)mca_parse_dts_u32(node, "ffc_temp_high", &info->ffc_temp_high,
				ALLOW_FFC_TEMP_HIGH_THR);
	...
	if (ret) {
		info->rx_max_iout[CHG_MODE_DIV2] =
			MCA_WLS_CHG_DEFAULT_DIV2_RX_MAX_IOUT;
...
static int
strategy_wireless_debug_process_data(const char *buf,
				     struct strategy_wireless_dev *info)
{
	char *pchar = NULL;
	int args[50] = { 0 };
	...
	case DEBUG_SET_FCC:
		...
		if (args[1] == 100000)
			mca_vote_override(info->charge_limit_voter, "wls_debug",
					  false, 0);
		else
			mca_vote_override(info->charge_limit_voter, "wls_debug",
					  true, args[1]);
	...
	case DEBUG_SET_ICL:
		...
		if (args[1] == 100000)
			mca_vote_override(info->input_limit_voter, "wls_debug",
					  false, 0);
		else
			mca_vote_override(info->input_limit_voter, "wls_debug",
					  true, args[1]);
	...
	case DEBUG_SET_ONE_EPP_FOD:
		...
		if (count == 4)
			platform_class_wireless_set_debug_fod(
				WIRELESS_ROLE_MASTER, args, count);
		break;
	...
	case DEBUG_SET_VOUT:
		...
		if (args[1] > 0 && args[1] <= 18000) {
			ret = platform_class_wireless_set_vout(
				WIRELESS_ROLE_MASTER, args[1]);
			if (ret < 0)
				mca_log_info(
					"wls debug set adapter voltage failed!!!\n");
		} else
...
static struct mca_sysfs_attr_info strategy_wireless_sysfs_field_tbl[] = {
	mca_sysfs_attr_rw(strategy_wireless_sysfs, 0664,
			  MCA_BASIC_CHG_WLS_DEBUG, wls_debug),
	mca_sysfs_attr_rw(strategy_wireless_sysfs, 0664,
			  MCA_BASIC_CHG_WLS_CAR_ADAPTER, wls_car_adapter),
	mca_sysfs_attr_rw(strategy_wireless_sysfs, 0664,
			  MCA_BASIC_CHG_WLS_FC_FLAG, wls_fc_flag),
	mca_sysfs_attr_rw(strategy_wireless_sysfs, 0664,
			  MCA_BASIC_CHG_WLS_LOW_INDUCTANCE_OFFSET,
			  low_inductance_offset),
	mca_sysfs_attr_rw(strategy_wireless_sysfs, 0664,
			  MCA_BASIC_CHG_WLS_SET_RX_SLEEP, set_rx_sleep),
	mca_sysfs_attr_rw(strategy_wireless_sysfs, 0664,
			  MCA_BASIC_CHG_WLS_AUDIO_PHONE_STS, audio_phone_sts),
};

#define MCA_BASIC_CHG_ATTRS_SIZE ARRAY_SIZE(strategy_wireless_sysfs_field_tbl)

static struct attribute
	*strategy_wireless_sysfs_attrs[MCA_BASIC_CHG_ATTRS_SIZE + 1];
					    ...
					    char *buf)
{
	...
	attr_info = mca_sysfs_lookup_attr(attr->attr.name,
					  strategy_wireless_sysfs_field_tbl,
	...
	case MCA_BASIC_CHG_WLS_DEBUG:
		(void)platform_class_wireless_get_vout(WIRELESS_ROLE_MASTER,
						       &rx_vout);
		(void)platform_class_wireless_get_vrect(WIRELESS_ROLE_MASTER,
							&rx_vrect);
		(void)platform_class_wireless_get_iout(WIRELESS_ROLE_MASTER,
						       &rx_iout);
					     ...
					     const char *buf, size_t count)
{
	...
	case MCA_BASIC_CHG_WLS_SET_RX_SLEEP:
		if (sscanf(buf, "%d\n", &info->wls_node.set_rx_sleep) != 1)
			return -1;
		mca_log_info("set rx sleep by user:%d",
			     info->wls_node.set_rx_sleep);
		platform_class_wireless_set_enable_mode(
			WIRELESS_ROLE_MASTER, (!info->wls_node.set_rx_sleep));
		break;
	case MCA_BASIC_CHG_WLS_AUDIO_PHONE_STS:
		(void)sscanf(buf, "%d", &info->audio_phone_sts);
		mca_log_info("set audio phone status:%d",
			     info->audio_phone_sts);
		mca_strategy_func_process(STRATEGY_FUNC_TYPE_QUICK_WIRELESS,
					  MCA_EVENT_WIRELESS_AUDIO_PHONE_STS,
					  info->audio_phone_sts);
		if (info->support_hall && info->proc_data.magnetic_case_flag &&
		    info->proc_data.fc_done)
			strategy_wireless_phone_sts_change(info);
		break;
	default:
		break;
...
{
	mca_sysfs_init_attrs(strategy_wireless_sysfs_attrs,
			     strategy_wireless_sysfs_field_tbl,
			     MCA_BASIC_CHG_ATTRS_SIZE);
	return mca_sysfs_create_link_group("charger", "wls_basic_charge", dev,
					   &strategy_wireless_sysfs_attr_group);
}

static void strategy_wireless_remove_group(struct device *dev)
{
	mca_sysfs_remove_link_group("charger", "wls_basic_charge", dev,
				    &strategy_wireless_sysfs_attr_group);
}
...
{
	INIT_DELAYED_WORK(&info->trans_data_work,
			  strategy_wireless_trans_data_work);
	INIT_DELAYED_WORK(&info->monitor_work, strategy_wireless_monitor_work);
	INIT_DELAYED_WORK(&info->find_voter_work,
			  strategy_wireless_find_voter_work);
	INIT_DELAYED_WORK(&info->report_soc_decimal_work,
			  strategy_wireless_update_soc_decimal_work);
	INIT_DELAYED_WORK(&info->renegociation_work,
			  strategy_wireless_renegociation_work);
	INIT_DELAYED_WORK(&info->wireless_loop_work,
			  strategy_wireless_loop_work);
	INIT_DELAYED_WORK(&info->rx_fastcharge_work,
			  strategy_wireless_rx_fastcharge_work);
	INIT_DELAYED_WORK(&info->wls_drawload_work,
			  strategy_wireless_drawload_work);
	INIT_DELAYED_WORK(&info->rx_alarm_work,
			  strategy_wireless_rx_alarm_work);
	INIT_DELAYED_WORK(&info->max_power_control_work,
			  strategy_wireless_max_power_control_work);
	INIT_DELAYED_WORK(&info->wls_fw_state_work,
			  strategy_wireless_fw_state_work);
	INIT_DELAYED_WORK(&info->set_vdd_flag_work,
			  strategy_wireless_set_vdd_flag_work);
	INIT_DELAYED_WORK(&info->soc_limit_stepper_work,
			  strategy_wireless_soc_limit_stepper_work);
	INIT_DELAYED_WORK(&info->update_wireless_thermal_work,
			  strategy_wireless_update_wireless_thermal_work);
	INIT_DELAYED_WORK(&info->mutex_unlock_work,
			  strategy_wireless_mutex_unlock_work);
...
static int strategy_wireless_init_voter(struct strategy_wireless_dev *info)
{
	info->input_limit_voter = mca_create_votable(
		"wireless_buck_input", MCA_VOTE_MIN,
	...
	info->bpp_in_voter = mca_create_votable("wireless_bpp_in", MCA_VOTE_MIN,
						strategy_wireless_set_bpp_input,
						1100, info);
	if (IS_ERR(info->bpp_in_voter))
		return -1;
	info->bppqc2_in_voter = mca_create_votable(
		"wireless_bppqc2_in", MCA_VOTE_MIN,
		strategy_wireless_set_bppqc2_input, 1100, info);
	if (IS_ERR(info->bppqc2_in_voter))
		return -1;
	info->bppqc3_in_voter = mca_create_votable(
		"wireless_bppqc3_in", MCA_VOTE_MIN,
	...
	info->epp_in_voter = mca_create_votable("wireless_epp_in", MCA_VOTE_MIN,
						strategy_wireless_set_epp_input,
						1100, info);
	if (IS_ERR(info->epp_in_voter))
		return -1;
	info->auth_20w_voter = mca_create_votable(
		"wireless_auth_20w", MCA_VOTE_MIN,
		strategy_wireless_set_auth_20w_ichg, 15600, info);
	if (IS_ERR(info->auth_20w_voter))
		return -1;
	info->auth_30w_voter = mca_create_votable(
		"wireless_auth_30w", MCA_VOTE_MIN,
		strategy_wireless_set_auth_30w_ichg, 15600, info);
	if (IS_ERR(info->auth_30w_voter))
		return -1;
	info->auth_50w_voter = mca_create_votable(
		"wireless_auth_50w", MCA_VOTE_MIN,
		strategy_wireless_set_auth_50w_ichg, 15600, info);
	if (IS_ERR(info->auth_50w_voter))
		return -1;
	info->auth_80w_voter = mca_create_votable(
		"wireless_auth_80w", MCA_VOTE_MIN,
		strategy_wireless_set_auth_80w_ichg, 15600, info);
	if (IS_ERR(info->auth_80w_voter))
		return -1;
	info->auth_voice_box_voter = mca_create_votable(
		"wireless_auth_voice_box", MCA_VOTE_MIN,
		strategy_wireless_set_auth_voice_box_ichg, 15600, info);
	if (IS_ERR(info->auth_voice_box_voter))
		return -1;
	info->auth_magnet_30w_voter = mca_create_votable(
		"wireless_auth_magnet_30w", MCA_VOTE_MIN,
		strategy_wireless_set_auth_magnet_30w_ichg, 15600, info);
	if (IS_ERR(info->auth_magnet_30w_voter))
		return -1;
	info->sw_qc_ichg_voter = mca_create_votable(
		"wireless_sw_qc_ich", MCA_VOTE_MIN,
		strategy_wireless_sw_set_qc_ichg, 15600, info);
	if (IS_ERR(info->sw_qc_ichg_voter))
		return -1;
	info->sw_thermal_ichg_voter = mca_create_votable(
		"wireless_sw_thermal_ich", MCA_VOTE_MIN,
		strategy_wireless_sw_set_thermal_ichg, 15600, info);
	if (IS_ERR(info->sw_thermal_ichg_voter))
		return -1;
	info->wls_input_suspend_voter =
		mca_create_votable("wireless_input_suspend", MCA_VOTE_OR,
				   strategy_wireless_input_suspend,
				   MCA_WLS_CHARGE_INPUT_SUSPEND, info);
	if (IS_ERR(info->wls_input_suspend_voter))
		return -1;
	info->force_vout_6v_voter = mca_create_votable(
		"wireless_force_vout_6v", MCA_VOTE_OR,
		strategy_wireless_force_set_vout_6v, 0, info);
	return 0;
}

static int strategy_wireless_soc_limit_sts_callback(void *data,
						    int effective_result)
{
	struct strategy_wireless_dev *info =
		(struct strategy_wireless_dev *)data;
	...
	if (effective_result && info->online)
		strategy_wireless_process_soc_limit_change(effective_result,
							   info);
	else {
...
static int strategy_wireless_quiet_sts_callback(void *data,
						int effective_result)
{
	struct strategy_wireless_dev *info =
		(struct strategy_wireless_dev *)data;
...
static int strategy_wireless_smartchg_delta_fv_callback(void *data,
							int effective_result)
{
	struct strategy_wireless_dev *info =
		(struct strategy_wireless_dev *)data;
