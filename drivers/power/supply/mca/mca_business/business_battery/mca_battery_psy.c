...
static int get_prop_batt_health(struct batt_psy_info *info,
...
static int get_prop_batt_capacity_level(struct batt_psy_info *info,
					union power_supply_propval *val)
{
	...
	power_supply_get_property(info->batt_psy, POWER_SUPPLY_PROP_CAPACITY,
				  &pval);

	if (pval.intval == 0) {
		val->intval = POWER_SUPPLY_CAPACITY_LEVEL_CRITICAL;
		mca_log_info(
			"Trigger Android shutdown for CAPACITY_LEVEL_CRITICAL\n");
				  ...
				  union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (battery_psy_is_usb_available(info)) {
			power_supply_get_property(
				info->usb_psy, POWER_SUPPLY_PROP_ONLINE, pval);
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		(void)platform_class_buckchg_ops_get_chg_type(MAIN_BUCK_CHARGER,
							      &main_chg_type);
			     ...
			     union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if (!info->input_suppend_voter)
			info->input_suppend_voter =
				mca_find_votable("input_suspend");
		if (info->input_suppend_voter &&
		    mca_get_client_vote(info->input_suppend_voter,
					"micharge")) {
		...
		if (pval->intval) {
			...
			rc = power_supply_get_property(
				info->wls_psy, POWER_SUPPLY_PROP_ONLINE, pval);
			if (pval->intval) {
				uisoc = strategy_class_fg_ops_get_soc();
				if (uisoc == 100 && charging_done)
					pval->intval = POWER_SUPPLY_STATUS_FULL;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		rc = power_supply_get_property(
			info->usb_psy, POWER_SUPPLY_PROP_CURRENT_MAX, pval);
		break;
...
static int battery_psy_init(struct batt_psy_info *info)
{
	...
	info->batt_psy = devm_power_supply_register(info->dev, &_batt_psy_desc,
						    &batt_cfg);
...
static int business_battery_dump_log_head(void *data, char *buf, int size)
{
	return snprintf(buf, size,
			"ChargeStatus uiSoc uiSoh F_Volt F_Curr  F_temp ");
}
...
static int business_battery_dump_log_context(void *data, char *buf, int size)
{
	...
	if (!info || !info->batt_psy)
		return snprintf(buf, size, "%-13d%-6d%-6d%-7d%-8d%-7d", -1, -1,
				-1, -1, -1, -1);

	(void)batt_psy_get_prop(info->batt_psy, POWER_SUPPLY_PROP_STATUS,
				&status);
	(void)batt_psy_get_prop(info->batt_psy, POWER_SUPPLY_PROP_CAPACITY,
				&soc);
	(void)batt_psy_get_prop(info->batt_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW,
				&volt);
	(void)batt_psy_get_prop(info->batt_psy, POWER_SUPPLY_PROP_CURRENT_NOW,
				&curr);
	(void)batt_psy_get_prop(info->batt_psy, POWER_SUPPLY_PROP_TEMP, &temp);

	return snprintf(buf, size, "%-13d%-6d%-6d%-7d%-8d%-7d", status.intval,
			soc.intval, soh, volt.intval / 1000, curr.intval / 1000,
			temp.intval);
}
