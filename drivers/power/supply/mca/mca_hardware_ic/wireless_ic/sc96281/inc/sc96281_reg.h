...
typedef struct fod_params_t {
	uint8_t params_nums;
	uint8_t type;
	uint8_t length;
	int uuid;
	fod_type_t params[DEFAULT_FOD_PARAM_LEN];
} fod_params_t;

typedef struct fw_params_t {
	uint8_t fw_rx_id;
	uint8_t fw_tx_id;
	uint8_t fw_boot_id;
	uint8_t hw_id_h;
...
struct sc96281 {
	...
	int hall_int_irq;
	//pinctrl
	struct pinctrl *sc96281_pinctrl;
	...
	struct sc96281_chg_proc_data proc_data;
	...
	fw_params_t *wls_fw_data;
	// fw_bin
	int fw_bin_length;
	unsigned char fw_bin[32768];
	// fod & debug_fod
	fod_params_t fod_params[FOD_PARA_MAX_GROUP];
	fod_params_t fod_params_default;
	uint8_t wls_debug_one_fod_index;
	int wls_debug_set_fod_type;
	fod_type_t wls_debug_one_fod_param;
	fod_params_t *wls_debug_all_fod_params;
