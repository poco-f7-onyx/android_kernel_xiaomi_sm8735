...
#ifndef MCA_LOG_TAG
#define MCA_LOG_TAG "sc96281_mtp_pgm"
#endif
...
static int SC96281_write_byte(struct sc96281 *sc, uint16_t reg, uint8_t data)
{
	int ret;
...
static int SC96281_read_register(struct sc96281 *sc, uint16_t reg,
...
static int sc96281_write_register(struct sc96281 *sc, uint16_t reg,
				  uint8_t *data, uint8_t len)
{
	int ret;
	uint8_t r_data[4] = { 0 };
...
static uint32_t sc96281_func_crc32(uint32_t data, uint32_t crc_init)
{
	...
	for (i = 0; i < 32; i++) {
		crc_init = (crc_init << 1) ^ ((((crc_init >> 31) & 0x01) ^
					       ((data >> i) & 0x01)) == 0x01 ?
						      0xFFFFFFFF & crc_poly :
						      0x00000000 & crc_poly);
	}
...
__maybe_unused static uint8_t pgm_state(struct sc96281 *sc)
{
	uint16_t i;
	uint8_t state[4] = { 0 };
	...
	for (i = 0; i < 1000; i++) {
		msleep(1);
		SC96281_read_register(sc, sram_addr_l + PGM_STATE_ADDR, state,
				      4);
		if ((state[0] != PGM_STATE_BUSY) &&
		    (state[0] != PGM_STATE_NONE)) {
			return state[0];
		}
	}
...
__maybe_unused static int pgm_sector_erase(struct sc96281 *sc, uint32_t addr,
					   uint16_t length)
{
	uint8_t state = 0;
	pgmPktType pgm = { 0 };
	...
	sc96281_write_register(sc, sram_addr_l + PGM_STATE_ADDR, &pgm.value[0],
			       PGM_HEADER_SIZE);
	sc96281_write_register(sc, sram_addr_l + PGM_CMD_ADDR,
			       write_single_data, 1);

	state = pgm_state(sc);
	if (state != PGM_STATE_DONE) {
		mca_log_err("Error: pgm sector erase state %02x\n", state);
		return -EINVAL;
	}
	return 0;
}

__maybe_unused static int pgm_chip_erase(struct sc96281 *sc)
{
	uint8_t state = 0;
	pgmPktType pgm = { 0 };
	uint16_t sram_addr_h = SRAM_BASE >> 16;
	...
	sc96281_write_block(sc, sram_addr_l + PGM_STATE_ADDR, &pgm.value[0],
			    PGM_HEADER_SIZE);
	...
	state = pgm_state(sc);
	if (state != PGM_STATE_DONE) {
		mca_log_err("Error: pgm chip erase state %02x\n", state);
		return -EINVAL;
	}
	return 0;
}

__maybe_unused static int pgm_trim_erase(struct sc96281 *sc)
{
	uint8_t state = 0;
	pgmPktType pgm = { 0 };
	uint16_t sram_addr_h = SRAM_BASE >> 16;
	...
	state = pgm_state(sc);
	if (state != PGM_STATE_DONE) {
		mca_log_err("Error: pgm trim erase state %02x\n", state);
		return -EINVAL;
	}
	return 0;
}

__maybe_unused static int pgm_cust_erase(struct sc96281 *sc)
{
	uint8_t state = 0;
	pgmPktType pgm = { 0 };
	uint16_t sram_addr_h = SRAM_BASE >> 16;
	...
	state = pgm_state(sc);
	if (state != PGM_STATE_DONE) {
		mca_log_err("Error: pgm cut erase state %02x\n", state);
		return -EINVAL;
	}
	return 0;
}

__maybe_unused static int pgm_set_margin(struct sc96281 *sc, uint8_t buffer)
{
	uint8_t state = 0;
	pgmPktType pgm = { 0 };
	uint16_t sram_addr_h = SRAM_BASE >> 16;
	...
	state = pgm_state(sc);
	if (state != PGM_STATE_DONE) {
		mca_log_err("Error: pgm access state %02x\n", state);
		return -EINVAL;
	}
	return 0;
}

__maybe_unused static uint16_t count_num(uint8_t *buffer, uint16_t len,
					 uint8_t num)
{
	uint16_t i;
	uint16_t count = 0;
...
__maybe_unused static int SC96281_get_pgm_info(struct sc96281 *sc, char *info)
{
	uint8_t state = 0;
	uint8_t i;
	uint8_t index;
	...
	sc96281_write_register(sc, sram_addr_l + PGM_STATE_ADDR, &pgm.value[0],
			       4);
	sc96281_write_register(sc, sram_addr_l + PGM_STATE_ADDR + 4,
			       &pgm.value[4], 4);
	...
	state = pgm_state(sc);
	if (state == PGM_STATE_DONE) {
		SC96281_read_register(sc, sram_addr_l + PGM_STATE_ADDR,
				      (uint8_t *)(&pgm.value[0]), 4);
		SC96281_read_register(sc, sram_addr_l + PGM_STATE_ADDR + 4,
				      (uint8_t *)(&pgm.value[4]), 4);
		mca_log_err("pgm.type.len = %d\n", pgm.type.len);
		msg_len = pgm.type.len;
		sc96281_read_block(sc, sram_addr_l + PGM_MSG_ADDR, pgm.type.msg,
				   msg_len);
		if (pgm_xor((uint8_t *)pgm.value, msg_len + PGM_HEADER_SIZE) ==
		    0x00) {
			index = 0;
			for (i = 0; i < msg_len; i++) {
				if (pgm.type.msg[i] != 0) {
		...
		mca_log_err("Error: pgm info state %02x\n", state);
		return -EINVAL;
	}
	return 0;
}

__maybe_unused static int pgm_access(struct sc96281 *sc, uint8_t *key_buffer,
				     uint8_t key_len)
{
	uint8_t state = 0;
	pgmPktType pgm = { 0 };
...
__maybe_unused static int pgm_read_crc(struct sc96281 *sc, uint8_t *crc,
				       uint16_t crc_size, uint16_t addr,
				       uint16_t len)
{
	...
	sc96281_write_block(sc, sram_addr_l + PGM_STATE_ADDR, &pgm.value[0],
			    PGM_HEADER_SIZE);
	SC96281_read_register(sc, sram_addr_l + PGM_STATE_ADDR, crc_verify_cmd,
			      4);
	SC96281_read_register(sc, sram_addr_l + PGM_STATE_ADDR, crc_verify_cmd,
			      4);

	crc_verify_cmd[1] = PGM_CMD_VERIFY;
	sc96281_write_register(sc, sram_addr_l + PGM_STATE_ADDR, crc_verify_cmd,
			       4);
	msleep(1);
	state = pgm_state(sc);
	if (state == PGM_STATE_DONE) {
		sc96281_read_block(sc, sram_addr_l + PGM_STATE_ADDR,
				   (uint8_t *)pgm.value, PGM_HEADER_SIZE);
		msg_len = crc_size;
		SC96281_read_register(sc, sram_addr_l + PGM_MSG_ADDR,
				      pgm.type.msg, msg_len);
		if (pgm_xor((uint8_t *)pgm.value, msg_len + PGM_HEADER_SIZE) ==
		    0x00) {
			index = 0;
		...
		mca_log_err("Error: pgm read crc state %02x\n", state);
		return -EINVAL;
	}
	return 0;
}

__maybe_unused static int pgm_read(struct sc96281 *sc, uint32_t addr,
				   uint8_t *buffer, uint16_t len, uint8_t sel)
{
	uint16_t i;
	uint8_t state = 0;
	pgmPktType pgm = { 0 };
	...
	for (i = 0; i < len; i += PGM_MSG_SIZE) {
		...
		sc96281_write_register(sc, sram_addr_l + PGM_STATE_ADDR,
				       &pgm.value[0], PGM_HEADER_SIZE);
		...
		if (state == PGM_STATE_DONE) {
			...
			mca_log_err("Error: pgm read state %02x\n", state);
			return -EINVAL;
		}
		p_buffer += offset;
	}
...
__maybe_unused static int pgm_write(struct sc96281 *sc, uint32_t addr,
				    uint8_t *buffer, uint16_t len, uint8_t sel)
{
	uint16_t i;
	uint8_t state = 0;
	...
	for (i = 0; i < len; i += PGM_FRIMWARE_SIZE) {
		...
		pgm.type.xor = pgm_xor(&pgm.value[PGM_XOR_INDEX],
				       offset + PGM_XOR_LEN);
		pgm.type.xor ^= (sel << 8);
		sc96281_write_block(sc, sram_addr_l + PGM_STATE_ADDR, pgm.value,
				    offset + PGM_HEADER_SIZE);
		...
		state = pgm_state(sc);
		if (state != PGM_STATE_DONE) {
			mca_log_err("Error: pgm write state %02x, cmd %02x\n",
				    state, sel);
			return -EINVAL;
		}
	}
	mca_log_err("firmware write successful\n");
	return 0;
}

static int bool_sel(struct sc96281 *sc, bool en)
{
	int ret;
	uint8_t data[4] = { 0 };
...
static int sys_reset_ctrl(struct sc96281 *sc, bool en)
{
	if (en) {
...
static int mcu_idle_ctrl(struct sc96281 *sc, bool en)
{
	...
	if (en) {
		...
		ret = sc96281_write_block(sc, ADDR_HIGH_BASE, idle_data + 2,
					  HIGH_ADDR_NUM);
		ret = sc96281_write_block(sc, idle_addr_low1, idle_result,
					  FOUR_BYTE_ALIGN);
		ret = sc96281_write_block(sc, idle_addr_low2, idle_clear,
					  FOUR_BYTE_ALIGN);
		msleep(5);
		ret = sc96281_write_block(sc, idle_addr_low1, idle_result,
					  FOUR_BYTE_ALIGN);
...
static int fp_size(struct file *f)
{
	...
	if (error == 0) {
		...
		mca_log_err("get file file stat error\n");
		return error;
	}
}
...
static int sc96281_read_bin(struct sc96281 *sc, char *firmware_buf,
...
static int sram_write(struct sc96281 *sc)
{
	int i;
	int ret;
	...
	mca_log_info("SC96281 pgm num = %d remaind = %d\n", pgm_pkg_num,
		     pgm_pkg_remaind);

	ret = mcu_idle_ctrl(sc, true);
	if (ret < 0) {
		mca_log_err("mcu_idle_ctrl failed\n");
		return -EINVAL;
	}
	...
	for (i = 0; i < size; i += PGM_PKG_SIZE) {
		if (pgm_pkg_count == pgm_pkg_num && pgm_pkg_remaind != 0) {
			ret = sc96281_write_block(
				sc, addr_base_l + i,
				(uint8_t *)&SC96281_PGM_DATA[i],
				(uint8_t)pgm_pkg_remaind);
			break;
		}
		ret = sc96281_write_block(sc, addr_base_l + i,
					  (uint8_t *)&SC96281_PGM_DATA[i],
					  PGM_PKG_SIZE);
		if (ret < 0) {
			mca_log_err("sc96281_write_block failed\n");
			return -EINVAL;
		}
		pgm_pkg_count++;
	}
	...
	if (ret < 0) {
		mca_log_err("sys_reset_ctrl failed\n");
		return ret;
	}

	ret = SC96281_get_pgm_info(sc, info);
	mca_log_err("info = %s\n", info);
	mca_log_err("pgm run sucessfully\n");

	return ret;
}

static int fw_program(struct sc96281 *sc, uint8_t *buf, uint32_t len)
{
	pgm_chip_erase(sc);
...
static int crc_check(struct sc96281 *sc, uint8_t margin, uint8_t *buf,
		     uint32_t len)
{
	int i, ret;
	uint32_t data32 = 0;
	...
	if (ret < 0) {
		mca_log_err("set margin fail\n");
		return ret;
	}

	ret = pgm_read_crc(sc, (uint8_t *)(&read_crc), 4, 0x0000,
			   CHIP_SIZE / PGM_WORD);
	if (ret < 0) {
		mca_log_err("pgm read crc failed");
		return -EINVAL;
	}

	data32 = 0xFFFFFFFF;
	for (i = 0; i < (CHIP_SIZE / PGM_WORD); i++) {
		data32 = sc96281_func_crc32(*(uint32_t *)(buf + i * PGM_WORD),
					    data32);
	}

	mca_log_info("margin data crc = 0x%08x read crc = 0x%08x\n", data32,
		     read_crc);
	if (data32 != read_crc) {
...
int mtp_program(struct sc96281 *sc, bool using_bin)
{
	int ret;
	...
	if (ret != 0 || firmware_buf == NULL) {
		mca_log_err("firmware get error %d\n", ret);
		goto program_fail;
	}
#else
	if (using_bin) {
