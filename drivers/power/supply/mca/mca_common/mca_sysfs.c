...
const char *g_mca_sysfs_dev_name_list[MCA_SYSFS_DEFAULT_DEV_NUM] = {
	"charger", "fuelgauge", "typec", "battery", "hw_monitor",
};

static struct mca_sysfs_class_node *
mca_sysfs_get_class_node(const char *cls_name)
{
...
mca_sysfs_get_device(struct mca_sysfs_class_node *class_node, const char *name)
{
				      ...
				      const struct attribute_group *group)
{
	...
	l_device = device_create(class_node->mca_class, NULL, 0, NULL, "%s",
				 dev_name);
	if (!l_device) {
			    ...
			    const struct attribute_group *group)
{
	...
	if (!class_node && !list_empty(&class_node->dev_list_header)) {
		list_for_each_entry_safe(dev_node, temp_node,
					 &class_node->dev_list_header, node) {
			if (dev_node->dev == dev) {
...
static int mca_sysfs_init(void)
{
	...
	for (i = 0; i < MCA_SYSFS_DEFAULT_DEV_NUM; i++) {
		...
		dev_node->dev = device_create(class_node->mca_class, NULL, 0,
					      NULL, "%s", dev_node->name);
		if (!dev_node->dev) {
