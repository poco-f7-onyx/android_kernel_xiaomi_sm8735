...
static struct mca_sysfs_attr_info mca_event_sysfs_field_tbl[] = {
	mca_sysfs_attr_wo(mca_event_sysfs, 0220, MCA_EVENT_SYSFS_TRIGGER,
			  trigger),
};
				     ...
				     const char *buf, size_t count)
{
	...
	info = mca_sysfs_lookup_attr(attr->attr.name, mca_event_sysfs_field_tbl,
				     MCA_EVENT_SYSFS_ATTRS_SIZE);
	if (!info)
		return -1;
