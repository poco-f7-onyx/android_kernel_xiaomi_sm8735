...
static struct mievent_upload_type_plug
	g_upload_type_plug_info[PLUG_TYPE_MAX_NUM] = {
		{ DEFAULT_MAX_COUNT, 0 },	{ LEVEL0_MAX_REPORT_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ DEFAULT_MAX_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ LEVEL0_MAX_REPORT_COUNT, 0 },
		{ LEVEL0_MAX_REPORT_COUNT, 0 }, { LEVEL0_MAX_REPORT_COUNT, 0 },
		{ LEVEL0_MAX_REPORT_COUNT, 0 }, { LEVEL0_MAX_REPORT_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ DEFAULT_MAX_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ DEFAULT_MAX_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ DEFAULT_MAX_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ LEVEL0_MAX_REPORT_COUNT, 0 },
		{ LEVEL0_MAX_REPORT_COUNT, 0 }, { DEFAULT_MAX_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ DEFAULT_MAX_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ DEFAULT_MAX_COUNT, 0 },
		{ LEVEL0_MAX_REPORT_COUNT, 0 }, { DEFAULT_MAX_COUNT, 0 },
		{ DEFAULT_MAX_COUNT, 0 },	{ DEFAULT_MAX_COUNT, 0 },
...
static struct mievent_upload_type_time
	g_upload_type_time_info[TIME_TYPE_MAX_NUM] = {
		{ 0, MINOR_TIMER_S, 0 },    { 0, CRITICAL_TIMER_S, 0 },
		{ 0, CRITICAL_TIMER_S, 0 }, { 0, BLOCK_TIMER_S, 0 },
		{ 0, BLOCK_TIMER_S, 0 },    { 0, BLOCK_TIMER_S, 0 },
		{ 0, BLOCK_TIMER_S, 0 },
	};
...
void mca_charge_mievent_report(int event_index, void *data, int size)
{
	...
	if (mievent->upload_type == MIEVENT_UPLOAD_TYPE_PLUG) {
		if (g_upload_type_plug_info[g_index].count <
		    g_upload_type_plug_info[g_index].max_count) {
			is_report = true;
		...
		if (!g_upload_type_time_info[g_index].count ||
		    time_now - g_upload_type_time_info[g_index].time_last >=
			    g_upload_type_time_info[g_index].time_interval) {
			is_report = true;
			mca_log_err("event_code[%d][%s] must report fault\n",
				    mievent->event_code,
				    mievent->event_describe);
	...
	if (is_report) {
		...
		cdev_tevent_add_str(event, mievent->event_type,
				    mievent->event_describe);
		mca_log_err("[%d] [%s] [%s]\n", mievent->event_code,
		...
		case MIEVENT_DATA_TYPE_INT:
			...
			for (int i = 0; i < mievent->data_count; i++) {
				cdev_tevent_add_int(
					event, mievent->para_name[i], param[i]);
				mca_log_err("[%s] [%d]\n",
		...
		case MIEVENT_DATA_TYPE_STRING:
			...
			for (int i = 0; i < mievent->data_count; i++) {
				if (strlen(string[i]) >=
				    MIEVENT_STRING_MAX_LEN) {
					mca_log_err(
						"[%s] [%s] String Len overflow\n",
						mievent->para_name[i],
						string[i]);
					continue;
				}
				cdev_tevent_add_str(event,
						    mievent->para_name[i],
						    string[i]);
				mca_log_err("[%s] [%s]\n",
				  ...
				  int value)
{
	...
	case MIEVENT_STATE_END:
		...
		if (mievent->upload_type == MIEVENT_UPLOAD_TYPE_PLUG)
			...
			g_upload_type_time_info[mievent->type_index].time_last =
				0;
