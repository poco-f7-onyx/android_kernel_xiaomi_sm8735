...
static int mca_get_client_id(struct mca_votable *votable,
	     ...
	     int val)
{
	...
	switch (votable->type) {
	case MCA_VOTE_MIN:
		mca_vote_min(votable, client_id, &effective_result,
			     &effective_id);
		break;
	case MCA_VOTE_MAX:
		mca_vote_max(votable, client_id, &effective_result,
			     &effective_id);
		break;
	case MCA_VOTE_OR:
		mca_vote_or(votable, client_id, &effective_result,
			    &effective_id);
		break;
	case MCA_VOTE_AND:
		mca_vote_and(votable, client_id, &effective_result,
			     &effective_id);
		break;
	default:
		return -EINVAL;
	...
	for (int i = 0; i < votable->num_clients && votable->client_strs[i];
	     i++) {
		int j = 0;
		      ...
		      bool enabled, int val)
{
	...
	if (enabled) {
		...
		rc = votable->callback(
			...
			mca_get_client_str(votable,
					   votable->effective_client_id));
		votable->override_result = -EINVAL;
		mca_log_err("%s cleared ovrried vote %d", override_client,
			    votable->override_result);
	}
...
static int mca_force_active_set(void *data, u64 val)
{
	...
	if (votable->force_active) {
		...
		if (votable->override_result != -EINVAL) {
			...
			effective_result = votable->effective_result;
			client = mca_get_client_str(
				votable, votable->effective_client_id);
		}
...
DEFINE_DEBUGFS_ATTRIBUTE(mca_votable_force_ops, mca_force_active_get,
			 mca_force_active_set, "%lld\n");
...
static int mca_show_votable_clients(struct seq_file *m, void *data)
{
	...
	seq_printf(m, "%s: effective=%s type=%s v=%d\n", votable->name,
		   ...
		   type_str, mca_get_effective_result_locked(votable));
	mca_unlock_votable(votable);
