...
static struct mca_charge_log_ops_data
	g_mca_charge_log_data[MCA_CHARGE_LOG_ID_MAX];
...
void __mca_log_err(const char *format, ...)
{
	...
	if (!info->info_level_flag)
		return;

	if (!info->init_flag) {
		va_start(args, format);
		len = vsnprintf(buff, MCA_LOG_SINGLE_LOG_MAX_LEN - 1, format,
				args);
		va_end(args);
	...
	len = snprintf(buff, MCA_LOG_SINGLE_LOG_MAX_LEN - 1,
		       "[%02d:%02d:%02d:%03ld-E]", cur_time.tm.tm_hour,
		       cur_time.tm.tm_min, cur_time.tm.tm_sec, cur_time.cur_ms);
	va_start(args, format);
	len += vsnprintf(buff + len, MCA_LOG_SINGLE_LOG_MAX_LEN - 1 - len,
			 format, args);
	va_end(args);
	pr_err("%s", buff);
...
void __mca_log_info(const char *format, ...)
{
	...
	mca_log_get_cur_time(&cur_time);
	len = snprintf(buff, MCA_LOG_SINGLE_LOG_MAX_LEN - 1,
		       "[%02d:%02d:%02d:%03ld-I]", cur_time.tm.tm_hour,
		       cur_time.tm.tm_min, cur_time.tm.tm_sec, cur_time.cur_ms);
	va_start(args, format);
	len += vsnprintf(buff + len, MCA_LOG_SINGLE_LOG_MAX_LEN - 1 - len,
			 format, args);
...
void __mca_log_debug(const char *format, ...)
{
	...
	mca_log_get_cur_time(&cur_time);
	len = snprintf(buff, MCA_LOG_SINGLE_LOG_MAX_LEN - 1,
		       "[%02d:%02d:%02d:%03ld-D]", cur_time.tm.tm_hour,
		       cur_time.tm.tm_min, cur_time.tm.tm_sec, cur_time.cur_ms);
	va_start(args, format);
	len += vsnprintf(buff + len, MCA_LOG_SINGLE_LOG_MAX_LEN - 1 - len,
			 format, args);
...
static struct mca_sysfs_attr_info mca_log_sysfs_field_tbl[] = {
	mca_sysfs_attr_rw(mca_log_sysfs, 0660, MCA_LOG_ATTR_LOG_LEVEL,
			  log_level),
	mca_sysfs_attr_rw(mca_log_sysfs, 0660, MCA_LOG_ATTR_LOG_ENABLE,
			  log_enable),
	mca_sysfs_attr_wo(mca_log_sysfs, 0220, MCA_LOG_ATTR_LOG_INDEX,
			  log_index),
	mca_sysfs_attr_ro(mca_log_sysfs, 0440, MCA_LOG_ATTR_LOG_CUR_BUFF,
			  cur_buff),
	mca_sysfs_attr_ro(mca_log_sysfs, 0440, MCA_LOG_ATTR_LOG_MAX_BUFF_NUM,
			  max_buffer_num),
	mca_sysfs_attr_ro(mca_log_sysfs, 0440, MCA_LOG_ATTR_DUMP_BUFFER_LOG,
			  dump_log_buff),
	mca_sysfs_attr_ro(mca_log_sysfs, 0440,
			  MCA_LOG_ATTR_DUMP_CHARGE_LOG_HEAD, charge_log_head),
	mca_sysfs_attr_ro(mca_log_sysfs, 0440,
			  MCA_LOG_ATTR_DUMP_CHARGE_LOG_INFO, charge_log_info),
	mca_sysfs_attr_rw(mca_log_sysfs, 0660, MCA_LOG_ATTR_CONSOLE_LOG_LEVEL,
			  console_log_level),
	mca_sysfs_attr_wo(mca_log_sysfs, 0220, MCA_LOG_ATTR_WRITE_LOG,
			  write_log),
};
...
static inline struct mca_charge_log_ops_data *
mca_log_get_charge_log_ops(unsigned int type)
{
...
static int mca_log_charge_log_dump_log_head(char *buf)
{
	...
	for (i = 0; i < MCA_CHARGE_LOG_ID_MAX; i++) {
		...
		len += ops_data->ops->dump_log_head(ops_data->data, buf + len,
						    PAGE_SIZE - len);
	}
...
static int mca_log_charge_log_dump_log_context(char *buf)
{
	...
	for (i = 0; i < MCA_CHARGE_LOG_ID_MAX; i++) {
		...
		len += ops_data->ops->dump_log_context(
			ops_data->data, buf + len, PAGE_SIZE - len);
	}
				   ...
				   const char *buf, size_t count)
{
	...
	info = mca_sysfs_lookup_attr(attr->attr.name, mca_log_sysfs_field_tbl,
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case MCA_LOG_ATTR_DUMP_BUFFER_LOG:
		...
		memcpy(buf, log_info->log_buff_cache[log_info->index],
		       PAGE_SIZE - 1);
		len = strlen(buf);
...
{
	mca_sysfs_remove_group("xm_power", info->dev,
			       &mca_log_sysfs_attr_group);
}
...
static void __exit mca_log_exit(void)
{
	...
	for (i = 0; i < MCA_LOG_BUFF_COUNT; i++) {
		if (info->log_buff_cache[i]) {
			kfree(info->log_buff_cache[i]);
			info->log_buff_cache[i] = NULL;
		}
	}
	kfree(info->log_buff);
	info->log_buff = NULL;
