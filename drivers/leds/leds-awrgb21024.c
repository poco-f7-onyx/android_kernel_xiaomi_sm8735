...
#include "leds-awrgb21024.h"
#include "leds-awrgb21024-reg.h"
...
struct aw21024_cfg aw21024e_effect_array[] = {
	{ aw21024_cfg_led_off, sizeof(aw21024_cfg_led_off) },
	{ aw21024e_leds_always_on, sizeof(aw21024e_leds_always_on) },
	{ aw21024_breath_leds_on, sizeof(aw21024_breath_leds_on) }
};
...
static int aw21024_i2c_write(struct aw21024 *aw21024, unsigned char reg_addr,
			     unsigned char reg_data)
{
	...
	while (cnt < AW_I2C_RETRIES) {
		ret = i2c_smbus_write_byte_data(aw21024->i2c, reg_addr,
						reg_data);
		if (ret < 0)
...
static int aw21024_i2c_read(struct aw21024 *aw21024, unsigned char reg_addr,
			    unsigned char *reg_data)
{
	int ret = -1;
	unsigned char cnt = 0;
 ...
 *
 *****************************************************/
static int aw21024_current_conversion(struct aw21024 *aw21024,
...
static int aw21024_brightness_conversion(struct aw21024 *aw21024,
					 unsigned int brightness_data)
{
	if (aw21024->cdev.max_brightness == 255) {
...
{
	if (flag) {
		aw21024_i2c_write_bits(aw21024, AW21024_REG_GCR,
...
static int aw21024_brres_set(struct aw21024 *aw21024, unsigned int mode)
{
	switch (mode) {
	case AW21024E_BRRES_8BITS:
		aw21024_i2c_write_bits(aw21024, AW21024_REG_GCR,
				       AW21024_BIT_BRRES_MASK,
				       AW21024_BIT_BRRES_8BITS);
		break;
	case AW21024E_BRRES_9BITS:
		aw21024_i2c_write_bits(aw21024, AW21024_REG_GCR,
				       AW21024_BIT_BRRES_MASK,
				       AW21024_BIT_BRRES_9BITS);
		break;
	case AW21024E_BRRES_12BITS:
		aw21024_i2c_write_bits(aw21024, AW21024_REG_GCR,
				       AW21024_BIT_BRRES_MASK,
				       AW21024_BIT_BRRES_12BITS);
		break;
	case AW21024E_BRRES_16BITS:
		aw21024_i2c_write_bits(aw21024, AW21024_REG_GCR,
				       AW21024_BIT_BRRES_MASK,
				       AW21024_BIT_BRRES_16BITS);
		break;
	default:
		break;
	}
...
static int aw21024_led_init(struct aw21024 *aw21024)
{
	...
	aw21024_brres_set(aw21024, AW21024E_BRRES_8BITS);
	...
	aw21024->run = false;
	aw21024->color.r = aw21024e_leds_always_on[LED_COLR_VALUE_INDEX];
	aw21024->color.g = aw21024e_leds_always_on[LED_COLG_VALUE_INDEX];
	aw21024->color.b = aw21024e_leds_always_on[LED_COLB_VALUE_INDEX];
	aw21024->color.timer[0] = 0x0f;
	aw21024->color.timer[1] = 0x0f;
	aw21024->color.timer[2] = 0x0f;
	aw21024->color.timer[3] = 0x0f;
...
static void aw21024_brightness_work(struct work_struct *work)
{
	struct aw21024 *aw21024 =
		container_of(work, struct aw21024, brightness_work);
	...
	if (aw21024->cdev.brightness > 0) {
		...
		aw21024_brightness_conversion(aw21024,
					      aw21024->cdev.brightness);
		aw21024_current_conversion(aw21024, aw21024->cdev.brightness);
...
static ssize_t rgbcolor_store(struct device *dev, struct device_attribute *attr,
			      const char *buf, size_t len)
{
	...
	if (sscanf(buf, "%x %x", &databuf[0], &databuf[1]) == 2) {
		...
		/* brightness default */
		aw21024_i2c_write(
			aw21024, AW21024_REG_GCCR,
			aw21024->conversion_led_current); //aw21024->conversion_led_current
		/* before trim */
		aw21024_i2c_write(aw21024, AW21024_REG_BR0_L + databuf[0] * 2,
				  0xff);
		/* after trim */
		/*aw21024_i2c_write(aw21024, AW21024_REG_BR0_L + databuf[0], 0xff);*/

		aw21024->rgbcolor = (databuf[1] & 0x00ff0000) >> 16;
		aw21024_i2c_write(aw21024, AW21024_REG_COL0 + databuf[0] * 3,
				  aw21024->rgbcolor);
		aw21024->rgbcolor = (databuf[1] & 0x0000ff00) >> 8;
		aw21024_i2c_write(aw21024,
				  AW21024_REG_COL0 + databuf[0] * 3 + 1,
				  aw21024->rgbcolor);
		aw21024->rgbcolor = (databuf[1] & 0x000000ff);
		aw21024_i2c_write(aw21024,
				  AW21024_REG_COL0 + databuf[0] * 3 + 2,
				  aw21024->rgbcolor);
			   ...
			   const char *buf, size_t len)
{
	...
	int rc = 0;
	rc = sscanf(buf, "%x", &val);
	pr_info("%s: color_store!!! %d\n", __func__, val);
	if (rc < 0)
		return rc;

	if (val < 0x1000000) {
		aw21024->color.r = (val & 0xff0000) >> 16;
		aw21024->color.g = (val & 0x00ff00) >> 8;
		aw21024->color.b = (val & 0x0000ff);
		if (aw21024->run) {
			aw21024_i2c_write(aw21024, AW21024_REG_GCOLR,
					  aw21024->color.r);
			aw21024_i2c_write(aw21024, AW21024_REG_GCOLG,
					  aw21024->color.g);
			aw21024_i2c_write(aw21024, AW21024_REG_GCOLB,
					  aw21024->color.b);
		}
	} else {
		pr_info("%s: invalid rgb color info!!!\n", __func__);
	}
	return len;
}
			    ...
			    const char *buf, size_t len)
{
	...
	unsigned int timerbuf[4] = { 0, 0, 0, 0 };

	if (sscanf(buf, "%d %d %d %d", &timerbuf[0], &timerbuf[1], &timerbuf[2],
		   &timerbuf[3]) == 4) {
		pr_info("%s: period_store!!! %d %d %d %d\n", __func__,
			timerbuf[0], timerbuf[1], timerbuf[2], timerbuf[3]);
		for (i = 0; i < 4; i++) {
			for (j = 2;
			     j < sizeof(breath_timerms_map_reg) /
					 sizeof(breath_timerms_map_reg[0]);
			     j += 2) {
				if (timerbuf[i] == 0) {
					aw21024->color.timer[i] = timerbuf[i];
				} else if (timerbuf[i] > 0 &&
					   timerbuf[i] <=
						   breath_timerms_map_reg[2]) {
					aw21024->color.timer[i] =
						breath_timerms_map_reg[3];
				} else if ((timerbuf[i] >=
					    breath_timerms_map_reg[j - 2]) &&
					   (timerbuf[i] <=
					    breath_timerms_map_reg[j])) {
					offset =
						(breath_timerms_map_reg[j] +
						 breath_timerms_map_reg[j - 2]) /
						2;
		...
		if (aw21024->run) {
			aw21024_i2c_write(aw21024, AW21024_REG_PATT0,
					  (aw21024->color.timer[0] & 0x0f)
							  << 4 |
						  aw21024->color.timer[1]);
			aw21024_i2c_write(aw21024, AW21024_REG_PATT1,
					  (aw21024->color.timer[2] & 0x0f)
							  << 4 |
						  aw21024->color.timer[3]);
		}
	} else {
		pr_info("%s: invalid rgb color timer info!!!\n", __func__);
	}
	return len;
}
			 ...
			 const char *buf, size_t len)
{
	...
	rc = kstrtouint(buf, 0, &val);
	pr_info("%s: run_store!!! %d\n", __func__, val);
	if (rc < 0)
		return rc;

	if (val <
	    (sizeof(aw21024e_effect_array) / sizeof(struct aw21024_cfg))) {
		switch (val) {
		case 1:
			aw21024e_leds_always_on[LED_CURRENT_INDEX] =
				aw21024->conversion_led_current;
			aw21024e_leds_always_on[LED_COLR_VALUE_INDEX] =
				aw21024->color.r;
			aw21024e_leds_always_on[LED_COLR_VALUE_INDEX] =
				aw21024->color.g;
			aw21024e_leds_always_on[LED_COLR_VALUE_INDEX] =
			...
			break;
		case 2:
			aw21024e_leds_breath_on[BREATH_CURRENT_INDEX] =
				aw21024->conversion_led_current;
			aw21024e_leds_breath_on[BREATH_TIMER_RISE_ON_INDEX] =
				(aw21024->color.timer[0] & 0x0f) << 4 |
				aw21024->color.timer[1];
			aw21024e_leds_breath_on[BREATH_TIMER_FALL_OFF_INDEX] =
				(aw21024->color.timer[2] & 0x0f) << 4 |
				aw21024->color.timer[3];
			aw21024e_leds_breath_on[BREATH_COLR_VALUE_INDEX] =
				aw21024->color.r;
			aw21024e_leds_breath_on[BREATH_COLG_VALUE_INDEX] =
				aw21024->color.g;
			aw21024e_leds_breath_on[BREATH_COLB_VALUE_INDEX] =
				aw21024->color.b;
			break;
		default:
			break;
		};
		aw21024_update_cfg_array(aw21024, aw21024e_effect_array[val].p,
					 aw21024e_effect_array[val].count);
		aw21024->run = val > 0 ? true : false;
	} else {
		pr_info("%s: invalid rgb color run info!!!\n", __func__);
	}
	return len;
}
			    ...
			    const char *buf, size_t len)
{
	...
	aw21024e_leds_breath_on[BREATH_REPEAT_NUM_L] = (unsigned char)val;

	if (aw21024->run) {
		aw21024_i2c_write(aw21024, AW21024_REG_PATT3,
				  aw21024e_leds_breath_on[BREATH_REPEAT_NUM_L] &
					  0xff);
	}
	return len;
}
			   ...
			   char *buf)
{
	...
	for (i = 0; i < sizeof(aw21024_cfg_array) / sizeof(struct aw21024_cfg);
	     i++) {
		len += snprintf(buf + len, PAGE_SIZE - len, "cfg[%x] = %ps\n",
...
static DEVICE_ATTR_WO(repeat);

static struct attribute *aw21024_attributes[] = {
	&dev_attr_reg.attr,    &dev_attr_hwen.attr,   &dev_attr_rgbcolor.attr,
	&dev_attr_color.attr,  &dev_attr_period.attr, &dev_attr_run.attr,
	&dev_attr_effect.attr, &dev_attr_repeat.attr, NULL
};
...
static int aw21024_parse_dts(struct device *dev, struct aw21024 *aw21024,
			     struct device_node *np)
{
	...
	ret = of_property_read_u32(np, "led_current",
				   &aw21024->dts_led_current);
	if (ret) {
...
static int aw21024_i2c_probe(struct i2c_client *i2c)
{
	...
	if (ret < 0) {
		dev_err(&i2c->dev,
			"%s: aw21024_read_chip_version failed ret=%d\n",
			__func__, ret);
		goto err_id;
	}
	...
	ret = sysfs_create_group(&aw21024->cdev.dev->kobj,
...
static const struct i2c_device_id aw21024_i2c_id[] = { { AW21024_I2C_NAME, 0 },
						       {} };
