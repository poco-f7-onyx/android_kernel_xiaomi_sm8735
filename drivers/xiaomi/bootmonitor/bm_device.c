...
int _partition_bm_read(struct block_device *dev, loff_t from, size_t len,
		       void *buf)
{
	struct page *page;
...
int get_bm_devices(void)
{
	...
	MTN_PRINT_INFO("%s-%d:bm dev: str %s:%zu\n", __func__, __LINE__, str,
		       strlen(str));
...
static void initerrortypeheader(void)
{
	...
	memset(error_type_header.error_record, 0,
	       sizeof(error_type_header.error_record));
...
static void initbootheader(void)
{
	...
	memset(boot_monitor_header.os_version, 0,
	       sizeof(boot_monitor_header.os_version));
...
int write_blackbox_header(struct bootmonitor_context *cxt, char *buffer,
			  int event)
{
	int err;
	int event_count;
	...
	err = _partition_bm_read(bdev, BLACK_BOX_OFFSET,
				 sizeof(struct log_table_header),
				 &black_box_log_table_header);
	if (err != 0) {
	...
	if (black_box_log_table_header.boot_content_offset !=
	    BOOT_CONTENT_OFFSET) {
		black_box_log_table_header.boot_content_offset =
			BOOT_CONTENT_OFFSET;
		MTN_PRINT_INFO(
			"%s-%d:black_box_log_table_header.boot_content_offset=0x%x\n",
			__func__, __LINE__,
			black_box_log_table_header.boot_content_offset);
	}
	//printlogtableheader();
	err = partition_bm_write(BLACK_BOX_OFFSET,
				 sizeof(struct log_table_header),
				 &black_box_log_table_header);
	if (err != 0) {
	...
	err = _partition_bm_read(bdev,
				 black_box_log_table_header.boot_content_offset,
				 sizeof(struct log_error_type_header),
				 &error_type_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:_partition_bm_read fail\n", __func__,
			      __LINE__);
	...
	strncpy(error_type_header.stage, CURRENT_BOOT_STAGE_INIT,
		sizeof(error_type_header.stage) - 1);
	error_type_header.stage[sizeof(error_type_header.stage) - 1] = '\0';
	strncpy(error_type_header.error_record, CURRENT_BOOT_INIT,
		sizeof(error_type_header.error_record) - 1);
	error_type_header
		.error_record[sizeof(error_type_header.error_record) - 1] =
		'\0';

	/*update error_header*/
	err = _partition_bm_read(
		bdev,
		black_box_log_table_header.boot_content_offset +
			error_type_header.header_size +
			INIT_INDEX * HEADER_SIZE_4K,
	...
	strncpy(error_header.error_record, bm_boot_events[bm_event_counts].name,
		sizeof(error_header.error_record) - 1);
	error_header.error_record[sizeof(error_header.error_record) - 1] = '\0';
	//printerrorheader();
	err = partition_bm_write(
		black_box_log_table_header.boot_content_offset +
			error_type_header.header_size +
			(INIT_INDEX + event_count) * HEADER_SIZE_4K,
		sizeof(struct log_error_header), &error_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:partition_bm_write fail\n", __func__,
			      __LINE__);
	...
	if (boot_monitor_header.magic != LOG_MONITOR_HEADER_MAGIC) {
		...
		strncpy(before_fingerprint, BEFORE_VERSION,
			sizeof(before_fingerprint) - 1);
		before_fingerprint[sizeof(before_fingerprint) - 1] = '\0';
	} else {
		/*record version change status*/
		MTN_PRINT_INFO(
			"build_fingerprint=%s, boot_monitor_header.os_version=%s\n",
			build_fingerprint, boot_monitor_header.os_version);
		if (strcmp(build_fingerprint, boot_monitor_header.os_version) ==
		    0)
			version_change = 0;
		else
			version_change = 1;
		...
		strncpy(before_fingerprint, boot_monitor_header.os_version,
			sizeof(boot_monitor_header.os_version) - 1);
		before_fingerprint[sizeof(before_fingerprint) - 1] = '\0';
	}
	boot_monitor_header.boot_index = black_box_log_table_header.boot_index;
	/*record this version*/
	strncpy(boot_monitor_header.os_version, build_fingerprint,
		sizeof(boot_monitor_header.os_version) - 1);
	boot_monitor_header
		.os_version[sizeof(boot_monitor_header.os_version) - 1] = '\0';
	/*record timeout event*/
	strncpy(boot_monitor_header.event, bm_boot_events[bm_event_counts].name,
		sizeof(boot_monitor_header.event) - 1);
	...
	err = partition_bm_write(black_box_log_table_header.boot_content_offset,
				 sizeof(struct log_error_type_header),
				 &error_type_header);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:partition_bm_write fail\n", __func__,
...
static void add_timeout_header(char *oops_buf)
{
	...
	ret_len = snprintf(
		str_buf, BOOT_MONITOR_HEADER_SIZE,
		"\n```\n## BOOT INDEX:%d\n## BOOT EVENTS:%s\n## BOOT MODE:%s\n##CURRENT VEISION:%s\n##BEFORE VEISION:%s\n"
		"## VEISION_CHANGE:%d\n##### %04ld-%02d-%02d %02d:%02d:%02d\n```c\n",
		bootindex, bm_boot_events[bm_event_counts].name, bm_boot_mode,
		build_fingerprint, before_fingerprint, version_change,
...
void monitor_get_kmsg(char *buffer)
{
	...
	MTN_PRINT_START();
	kmsg_dump_get_buffer(&iter, true, buffer + BOOT_MONITOR_HEADER_SIZE,
			     PARTITION_KMSG_SIZE - HEADER_SIZE_4K -
				     BOOT_MONITOR_HEADER_SIZE,
			     &ret_len);
...
void monitor_get_pmsg(struct bootmonitor_context *cxt)
{
	...
	if (p_hdr->sig == 0x43474244) {
		...
		uint8_t *p_buff_end =
			(uint8_t *)p_hdr->data + atomic_read(&p_hdr->size);
		int pmsg_cp_size = 0;
...
#define parse_u32(name, field, default_value)                                  \
	{                                                                      \
		ret = monitor_parse_dt_u32(pdev, name, default_value, &value); \
		if (ret < 0)                                                   \
			return ret;                                            \
		field = value;                                                 \
	}
...
#undef parse_u32
	...
	MTN_PRINT_INFO(
		"pares, mem_address =0x%llx, mem_size =0x%lx, console-size =0x%lx, pmsg-size =0x%lx\n",
		cxt->monitor_data.mem_address, cxt->monitor_data.mem_size,
