...
MODULE_PARM_DESC(blackbox, "blackbox (default 0)");
...
static bool find_event(int nums)
{
	...
	for (i = ringnums; i < log_count_2; i++) {
		...
		occurrence = strstr(p->comm_event + TASK_COMM_LEN,
...
static int write_log_to_dev(struct bootmonitor_context *cxt, char *buffer,
			    int event)
{
	int err;
	...
	err = partition_bm_write(PARTITION_KMSG_OFFSET + HEADER_SIZE_4K,
				 PARTITION_KMSG_SIZE - HEADER_SIZE_4K, buffer);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:partition_bm_write error\n", __func__,
			      __LINE__);
		return err;
	}
	err = partition_bm_write(PARTITION_PMSG_OFFSET, PARTITION_PMSG_SIZE,
				 cxt->oops_buf);
	if (err != 0) {
		MTN_PRINT_ERR("%s-%d:partition_bm_write error\n", __func__,
			      __LINE__);
...
static int monitor_main_thread_body(void *data)
{
	...
	while (!kthread_should_stop()) {
		for (bm_event_counts = 0;
		     bm_event_counts < countof(bm_boot_events);
		     bm_event_counts++) {
			counts = bm_boot_events[bm_event_counts].warntime + 1;
			MTN_PRINT_INFO(
				"counts = bm_boot_events[%d].warntime = %d\n",
				bm_event_counts,
				bm_boot_events[bm_event_counts].warntime);
			for (k = 0; k < counts; k++) {
				...
				if (!ret && k != (counts - 1)) {
					...
					continue;
					   ...
					   k == bm_boot_events[bm_event_counts]
							   .warntime) {
					strncat(bootwarn,
						bm_boot_events[bm_event_counts]
							.name,
						sizeof(bm_boot_events
							       [bm_event_counts]
								       .name));
					//bm_sendnlmsg(bootwarn);
					/*reset bootwarn*/
					memset(bootwarn, 0, sizeof(bootwarn));
					snprintf(bootwarn, sizeof(bootwarn),
						 "warning:");
					/*set counts to error time*/
					counts = bm_boot_events[bm_event_counts]
							 .errortimes +
						 1;
					MTN_PRINT_INFO(
						"counts = bm_boot_events[%d].errortimes = %d\n",
						bm_event_counts,
						bm_boot_events[bm_event_counts]
							.errortimes);
					/*if first boot, ams ready time to long*/
					if (first_boot == 1 &&
					    bm_event_counts == AMS_READY) {
					...
					MTN_PRINT_INFO(
						"%s timeout:boottime is %d line=%d\n",
			...
			if ((k == bm_boot_events[bm_event_counts].errortimes ||
			     boottime == DEFAULT_BOOT_TIME) &&
			    !ret) {
				strncat(booterror,
					bm_boot_events[bm_event_counts].name,
					sizeof(bm_boot_events[bm_event_counts]
						       .name));
				strncpy(prof_node, booterror,
					sizeof(booterror));
				//bm_sendnlmsg(booterror);
				if (!get_bm_devices()) {
					err = write_log_to_dev(cxt,
							       bm_write_buffer,
							       bm_event_counts);
				}
...
static ssize_t boot_monitor_write(struct file *filp, const char *ubuf,
...
static int __init boot_monitor_init(void)
{
	...
	if (blackbox != BLACKBOX_VERSION) {
		MTN_PRINT_ERR(
			"%s-%d:blackbox  is %d, BLACKBOX_VERSION is %d, no bootfail partition\n",
	...
	if (result != 0) {
		MTN_PRINT_ERR(
			"%s-%d:boot BOOT_MODE_NORMAL is %s,no need boot monitor\n",
	...
	bootmonitor_entry =
		proc_create("boot_monitor", 0664, NULL, &boot_monitor_fops);
	...
	monitor_main =
		kthread_run(monitor_main_thread_body, NULL, "monitor_main");
	if (!monitor_main) {
		if (bm_write_buffer)
			vfree(bm_write_buffer);
		if (cxt->oops_buf)
...
static void __exit boot_monitor_exit(void)
{
	...
	if (bm_write_buffer) {
		MTN_PRINT_INFO("%s-%d:bm_write_buffer is %p\n", __func__,
			       __LINE__, bm_write_buffer);
		vfree(bm_write_buffer);
	}
	if (cxt->oops_buf) {
		MTN_PRINT_INFO("%s-%d:cxt->oops_buf is %p\n", __func__,
			       __LINE__, cxt->oops_buf);
		vfree(cxt->oops_buf);
	}
	if (bootmonitor_entry) {
