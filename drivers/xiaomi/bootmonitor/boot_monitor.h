...
#define MTN_PRINT_START(args...) \
	pr_info(">>>enter  %s: line: %d.\n", __func__, __LINE__);
#define MTN_PRINT_END(args...) \
...
extern int _partition_bm_read(struct block_device *dev, loff_t from, size_t len,
			      void *buf);
...
extern int write_blackbox_header(struct bootmonitor_context *cxt, char *buffer,
				 int event);
