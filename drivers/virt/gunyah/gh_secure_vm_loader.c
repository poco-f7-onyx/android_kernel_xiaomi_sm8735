							...
							struct gh_vm *vm)
{
	...
	if (ret || vmid != GH_VMID_INVAL) {
		dev_err(dev,
			"vmid as %d already allocated or get error for %s with ret %d\n",
	...
	if (!vm_dev->is_static) {
		if (vm_dev->fw_virt) {
			...
			pr_info("Allocate CMA memory for %s\n",
				vm_dev->vm_name);
			virt = dma_alloc_coherent(dev, vm_dev->fw_size,
						  &dma_handle, GFP_KERNEL);
			if (!virt) {
				ret = -ENOMEM;
				dev_err(dev,
					"Failed to allocate cma memory for %s %d\n",
					vm_dev->vm_name, ret);
				return ret;
			}
...
static int gh_vm_loader_mem_probe(struct gh_sec_vm_dev *sec_vm_dev)
{
	...
	if (!of_property_read_bool(node, "no-map")) {
		...
		do {
			virt = dma_alloc_coherent(dev, sec_vm_dev->fw_size,
						  &dma_handle, GFP_KERNEL);
			if (virt) {
				break;
			}
		...
		if (!virt) {
			ret = -ENOMEM;
			dev_err(dev,
				"Couldn't allocate cma memory %d times for %s with %d\n",
