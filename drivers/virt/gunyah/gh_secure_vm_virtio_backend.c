...
loop_back:
	...
	case GH_GET_QUEUE_INFO:
		...
		dev_dbg(vm->dev, "%s: get_queue_info %d: que_num %lld que_ready %lld que_desc %llx\n",
...
static int alloc_vm_shm(struct virt_machine *vm)
{
	...
	for (i = 0; i < vm->shmem_entries; ++i) {
		retry_time = 0;
		do {
			vm->shmem[i].base = dma_alloc_coherent(
				vm->com_mem_dev, vm->shmem[i].size, &dma_handle,
				GFP_KERNEL);
			if (vm->shmem[i].base) {
				break;
			}
		...
		vm->shmem[i].r.end =
			vm->shmem[i].r.start + vm->shmem[i].size - 1;
		dev_info(
			vm->dev,
			"%s: Pre-allocate VM '%s' shmem index %d shmem_size %x\n",
...
free_shm:
	for (; j >= 0; --j)
		dma_free_coherent(
			vm->com_mem_dev, vm->shmem[j].size, vm->shmem[j].base,
...
new_vm(struct device *dev, const char *vm_name, struct device_node *np)
{
	...
	if (vm->is_static) {
		...
		if (ret) {
			dev_err(vm->dev,
				"%s: Failed to alloc virtio backend shared memory size for VM %s\n",
				VIRTIO_PRINT_MARKER, vm_name);
			return NULL;
		}
...
static int share_vm_buffers(struct virt_machine *vm, gh_vmid_t peer)
{
	...
	for (i = 0; i < vm->shmem_entries; ++i) {
		if (!vm->is_static && !vm->shm_allocated) {
			...
			do {
				vm->shmem[i].base = dma_alloc_coherent(
					vm->com_mem_dev, vm->shmem[i].size,
			...
			dev_info(
				vm->dev,
				"%s: Runtime-allocate VM '%s' virtio backend shmem idx %d \n",
